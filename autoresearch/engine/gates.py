...
def _gate_inline_definition_preservation(original: str, new_content: str,
    ...
    Also detects definitions attached to technical terms (abbreviations, backtick-
    quoted terms) that aren't bolded — e.g. 'NVLink/PCIe — the physical GPU-to-GPU
    interconnects' or '`device_group` (NCCL process group for GPU collectives)'.
    ...
    """
    # Match all three definition formats (bold terms)
    defn_re = re.compile(
        r'\*\*([^*]+)\*\*\s*(?:'
    ...
    # Match definitions attached to non-bold technical terms:
    # - Abbreviations: 2+ uppercase letters (NVLink, PCIe, NCCL, HBM, SRAM, etc.)
    # - Backtick-quoted terms: `device_group`, `cpu_group`, etc.
    # These often appear in table cells and prose without bolding.
    _ABBREV_RE = re.compile(
        r'(?:'
        r'([A-Z][A-Za-z]*[A-Z][A-Za-z/]*)'  # mixed-case abbreviation (NVLink, PCIe)
        r'|`([^`]{2,})`'                      # backtick-quoted term
        r')\s*(?:'
        r'\(([^)]{10,})\)'                     # (parenthetical definition)
        r'|—\s*(.{10,}?)(?:[.\n|]|$)'         # — em-dash definition (stop at period, newline, or pipe)
        r')',
    )
    orig_prose = _strip_code_blocks(original)
    new_prose = _strip_code_blocks(new_content)
    ...
    # Common English function words/phrases used for bold emphasis, not jargon definitions.
    # These must not be treated as technical terms requiring inline definitions.
    _EMPHASIS_WORDS = frozenset({
        "but", "and", "or", "nor", "so", "yet", "for",   # conjunctions
        "note", "warning", "caution", "important", "tip",  # callout labels
        "however", "therefore", "thus", "hence", "moreover", "furthermore",
        "alternatively", "additionally", "specifically", "notably",
        "result", "outcome", "problem", "solution", "example",
    })

    # Adjectives that introduce rhetorical structural labels, not jargon terms.
    # "Key difference", "Main advantage", "Core insight" etc. are structural labels
    # even when bolded — the following parenthetical is the content, not a definition.
    _RHETORICAL_LEAD_WORDS = frozenset({
        "key", "main", "core", "primary", "fundamental", "critical",
        "important", "notable", "major", "primary", "secondary",
    })

    def _is_interview_question(term: str) -> bool:
        """Return True if the bold term is an interview question or common emphasis word."""
        t = term.strip()
        if t.startswith('"') or t.startswith("'") or t.endswith('?') or t.endswith('?"'):
            return True
        t_lower = t.lower()
        # Single common English words used for emphasis/formatting, not as jargon
        if t_lower in _EMPHASIS_WORDS:
            return True
        # Multi-word rhetorical labels like "Key difference", "Main advantage" —
        # these are structural labels, not technical terms being defined.
        first_word = t_lower.split()[0] if t_lower.split() else ""
        if first_word in _RHETORICAL_LEAD_WORDS and len(t_lower.split()) > 1:
            return True
        return False
    ...
    def _extract_definitions(text: str) -> dict[str, str]:
        ...
        Extracts from both bold terms (**term** (def)) and non-bold technical
        terms (NVLink/PCIe — def, `device_group` (def)). Table rows are included
        for non-bold extraction since table cells often contain definitions.
        """
        clean_no_tables = _strip_table_rows(text)
        defs = {}
        # Bold-term definitions (exclude table rows to avoid false matches)
        for m in defn_re.finditer(clean_no_tables):
            term = m.group(1).strip()
            if _is_interview_question(term):
                continue
            defn = m.group(2) or m.group(3) or m.group(4) or ""
            defs[_normalize_term(term)] = defn.strip()
        # Non-bold technical term definitions (include table rows)
        for m in _ABBREV_RE.finditer(text):
            term = (m.group(1) or m.group(2)).strip()
            defn = (m.group(3) or m.group(4) or "").strip()
            if not defn:
                continue
            norm = _normalize_term(term)
            # Don't overwrite a bold-term definition with an abbreviation one
            if norm not in defs:
                defs[norm] = defn
        return defs
    ...
    # Also check non-bold technical terms with definitions in new content
    new_abbrev_with_defn = set()
    for m in _ABBREV_RE.finditer(new_prose):
        term = (m.group(1) or m.group(2)).strip()
        new_abbrev_with_defn.add(_normalize_term(term))
    ...
    # Also checks non-bold occurrences for abbreviation/backtick terms.
    def _defn_info_present(norm_term: str, orig_defn: str, new_prose: str) -> bool:
        ...
        if not keywords:
            return False
        # Find all positions where the normalized term appears (bold or non-bold) in new_prose
        # Bold occurrences
        positions = []
        for m in re.finditer(r'\*\*([^*]+)\*\*', new_prose):
            if _normalize_term(m.group(1)) == norm_term:
                positions.append((m.start(), m.end()))
        # Non-bold abbreviation/backtick occurrences
        for m in re.finditer(r'(?:([A-Z][A-Za-z]*[A-Z][A-Za-z/]*)|`([^`]{2,})`)', new_prose):
            term = (m.group(1) or m.group(2)).strip()
            if _normalize_term(term) == norm_term:
                positions.append((m.start(), m.end()))
        for start_pos, end_pos in positions:
            # Check if definition keywords appear within 200 chars of this occurrence
            window_start = max(0, start_pos - 50)
            window_end = min(len(new_prose), end_pos + 200)
            window = new_prose[window_start:window_end].lower()
    ...
    for norm_term, defn in orig_definitions.items():
        if norm_term in new_keys or norm_term in new_bold_with_defn or norm_term in new_abbrev_with_defn:
            continue
...
def _gate_wikilink_count(original: str, new_content: str, result: GateResult) -> None:
    """Veto if unique wikilink targets decrease (net link loss).
...
# ---------------------------------------------------------------------------
# Index completeness gate (for maintain_index strategy)
# ---------------------------------------------------------------------------

def gate_index_completeness(index_content: str, all_notes_in_domain: list[str]) -> GateResult:
    """Check that index lists all domain notes with valid links.

    Args:
        index_content: the proposed index note content.
        all_notes_in_domain: list of relative paths (without .md) for all notes in the domain.

    Returns:
        GateResult — fails if notes are missing or dangling links exist.
    """
    result = GateResult()
    index_links = set(re.findall(r'\[\[([^\]|]+?)(?:\|[^\]]+?)?\]\]', index_content))
    expected = set(all_notes_in_domain)

    missing = expected - index_links
    dangling = index_links - expected

    if missing:
        result.fail(f"Index missing notes: {missing}")
    if dangling:
        result.fail(f"Index has dangling links: {dangling}")
...
def check_all_gates(original_content: str, new_content: str,
    ...
    is_cross_file = strategy in ("split", "dedup", "cross_link", "rename", "consolidate")
