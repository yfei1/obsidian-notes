...
def generate_delta(target_path: str, content: str, strategy: Strategy,
    ...
    """
    max_tokens = 16384 if strategy.name in ("split", "dedup", "cross_link", "systematize", "rename") else 8192
    timeout = 600 if strategy.name in ("split", "dedup", "cross_link", "systematize", "rename") else 300

    def llm_fn(prompt: str) -> str | None:
        return call_claude(prompt, model="sonnet", max_tokens=max_tokens, temperature=0.7, timeout=timeout)
...
# ---------------------------------------------------------------------------
# Index helpers (for maintain_index strategy)
# ---------------------------------------------------------------------------

def build_prereq_list(all_notes: list[Path], file_contents: dict[str, str]) -> str:
    """Build prereq-annotated note list for strategy context.

    Format: '- topic/subdir/note → prereqs: dep1, dep2' per line.
    """
    lines = []
    for note_path in all_notes:
        rel = relative_path(note_path).replace('.md', '')
        content = file_contents.get(str(note_path), '')
        prereq_line = next(
            (l for l in content.splitlines()[:10] if 'Prerequisites' in l), None
        )
        prereqs = ""
        if prereq_line:
            links = re.findall(r'\[\[([^\]]+)\]\]', prereq_line)
            prereqs = ", ".join(l.split('/')[-1] for l in links)
        if prereqs:
            lines.append(f"- {rel} → prereqs: {prereqs}")
        else:
            lines.append(f"- {rel}")
    return "\n".join(lines)


def build_note_summaries(all_notes: list[Path], file_contents: dict[str, str]) -> str:
    """Extract TL;DR (capped at 200 chars) for index strategy context."""
    sections = []
    for note_path in all_notes:
        rel = relative_path(note_path).replace('.md', '')
        content = file_contents.get(str(note_path), '')
...
def _score_strategy(strategy: Strategy, history: list[dict],
    ...
    """
    # Per-note filter: skip strategies that were vetoed, invalid, or exhausted on this note
    note_outcome = note_tried.get(strategy.name)
    if note_outcome in ("vetoed", "invalid", "unavailable"):
        return None
    # After 3 identity wins, the strategy consistently fails on this note — skip it
    if isinstance(note_outcome, int) and note_outcome >= 3:
        return None
...
def select_target(notes: list[Path], history: list[dict],
                   cached_scores: dict[str, dict[str, dict]] | None = None,
                   note_tried: dict[str, dict[str, str]] | None = None,
                   strategy_filter: str | None = None) -> Path | None:
    ...
    for note in notes:
        ...
        # Skip notes where the filtered strategy is exhausted
        if strategy_filter and note_tried:
            outcome = note_tried.get(rp, {}).get(strategy_filter)
            if outcome in ("unavailable", "vetoed", "invalid"):
                continue
            if isinstance(outcome, int) and outcome >= 3:
                continue
        ...
        if cached_scores and rp in cached_scores:
            ...
            # No LLM scores cached (note was edited this run or not yet scored).
            # Rule-based dims (Cross-Linking, Uniqueness, Code Quality) score 2–4
            # for notes with legitimate cross-linking and must NOT drive crisis
            # detection or targeting priority — they don't reflect content quality.
            avg = 7.0  # neutral until LLM rescore
            subj_avg = None
            subj_valid = []
        ...
        # Only declare crisis when we have actual LLM-scored subjective dims.
        # Without cached LLM scores, subj_avg is None — don't fire crisis on
        # rule-based fallback (Cross-Linking/Uniqueness score 2–4 legitimately).
        in_crisis = subj_avg is not None and priority_avg < CRISIS_AVG_THRESHOLD
...
def _check_gates_cross_file(winner: Delta, file_contents: dict[str, str],
    ...
    for path in winner.affected_paths():
        ...
        if baseline_for_path is None and winner.strategy in ("split", "rename") and baseline_violations_map:
            # New file created by split/rename — inherit the parent file's baseline violations.
            # The parent is whichever affected path already exists in file_contents.
            # For rename: the new path IS the old file, just moved — same violations apply.
            # For split: duplicate headers / line-limit issues came from the source material.
            parent_path = next(
...
def run_evolution(max_gen: int = MAX_GENERATIONS, group_size: int = GROUP_SIZE,
                  push: bool = True, target_filter: str | None = None,
    ...
    for generation in range(1, max_gen + 1):
        ...
        all_notes = [n for n in discover_notes() if not is_index_note(n)]

        # ── LOAD ALL FILE CONTENTS (for multi-file ops + overlap pre-filtering) ──
        file_contents = {relative_path(n): read_note(n) for n in all_notes}

        # ── SELECT TARGET ──
        filtered_notes = all_notes
        if target_filter:
            filtered_notes = [n for n in all_notes if target_filter in relative_path(n)]
            if not filtered_notes:
                print(f"  No notes match target filter '{target_filter}', skipping.")
                continue
        ...
        if strategy_filter == "dedup":
            ...
            for o in _pre_overlaps:
                ...
                _overlap_notes.add(o.canonical_path)
        ...
        if target_note is None:
            print(f"\n  All notes exhausted for strategy '{strategy_filter}'. Stopping early.")
            break
        ...
        if target_overlap:
            # Extend previews beyond the 150-char detection limit so the LLM
            # can locate the exact duplicated region in a long note.
            _src_preview = target_overlap.source_preview
            _can_preview = target_overlap.canonical_preview
            # Re-extract longer previews from actual content if possible
            for _preview_text, _full_content, _attr in [
                (_src_preview, target_content, "source"),
                (_can_preview, file_contents.get(target_overlap.canonical_path, ""), "canonical"),
            ]:
                # Find the paragraph containing the preview and grab up to 400 chars
                _start = _full_content.find(_preview_text[:60])
                if _start >= 0:
                    _end = min(_start + 400, len(_full_content))
                    _extended = _full_content[_start:_end].replace('\n', ' ').strip()
                    if _attr == "source":
                        _src_preview = _extended
                    else:
                        _can_preview = _extended
            dedup_vars = {
                "canonical_note": target_overlap.canonical_path,
                "overlap_preview": _src_preview,
                "canonical_preview": _can_preview,
            }
        ...
        sibling_names = "\n".join(
            f"- {Path(relative_path(n)).stem}"
            for n in all_notes
            if str(Path(relative_path(n)).parent) == target_dir
            and relative_path(n) != target_path
        )
        candidate_pool.append((RENAME_STRATEGY, {"sibling_names": sibling_names or "(no siblings)"}))
        ...
        if intra_overlaps:
            ...
            def _overlap_priority(o):
                b = o.section_b.lower()
                if "see also" in b or "connections" in b:
                    return (0, -o.overlap_ratio)  # best: duplicate links
                if "interview" in b or "talking" in b:
                    return (1, -o.overlap_ratio)  # good: ITP restating body
                return (2, -o.overlap_ratio)      # harder: body-body
            best = sorted(intra_overlaps, key=_overlap_priority)[0]
            candidate_pool.append((CONDENSE_STRATEGY, {
                "section_a": best.section_a,
                "section_b": best.section_b,
        ...
        # Lookup for post-rank retry: strategy name → (Strategy, extra_vars)
        strategy_lookup: dict[str, tuple] = {s.name: (s, ev) for s, ev in strategies_to_run}
        post_rank_retried = False  # one post-rank retry per generation iteration
        ...
        def _gen_one(strategy_and_vars):
            ...
            if ops is None:
                ...
                if retry_fb:
                    ...
                    ops, retry_fb2 = generate_delta(
                        target_path, target_content, strategy, constitution,
                        error_feedback=retry_fb,
                        extra_vars=extra_vars,
                    )
                    # Second retry for concretize: verification may fail again with
                    # different numbers — feed the new error back for one more attempt
                    if ops is None and retry_fb2 and strategy.name == "concretize":
                        print(f"  {strategy.name}: 2nd retry (verification still failing)...", flush=True)
                        ops, _ = generate_delta(
                            target_path, target_content, strategy, constitution,
                            error_feedback=retry_fb2,
                            extra_vars=extra_vars,
                        )
                if ops is None:
            ...
            # Gate check: structural validation before this delta enters judging.
            # If gates fail, retry once with the violation reasons as error_feedback
            # so the strategy can produce a structurally valid edit instead.
            def _build_note_paths(ops):
                paths = [relative_path(n) for n in all_notes]
                for op in ops:
                    if op.kind == "create_file" and op.path not in paths:
                        paths.append(op.path)
                    if op.kind == "rename_file" and op.new_path not in paths:
                        paths.append(op.new_path)
                return paths

            def _gate_check(d, d_new_contents):
                d_note_paths = _build_note_paths(d.ops)
                d_baseline_map: dict[str, set[str]] = {}
                for _p in d.affected_paths():
                    _orig = file_contents.get(_p, "")
                    if _orig and not is_conforming(_orig):
                        _bl = check_all_gates(_orig, _orig, _p, d_note_paths, strategy="")
                        d_baseline_map[_p] = set(_bl.violations)
                return _check_gates_cross_file(
                    d, file_contents, d_new_contents, d_note_paths,
                    baseline_violations_map=d_baseline_map or None,
                )

            d_new_contents, _ = delta.execute_all(file_contents)
            gate_failed, violations = _gate_check(delta, d_new_contents)
            if gate_failed:
                violations_str = "; ".join(violations)
                print(f"  {strategy.name}: gate violation — retrying with feedback...", flush=True)
                gate_fb = (
                    f"Your edit failed structural validation:\n{violations_str}\n\n"
                    f"Revise your edit to fix these violations. Preserve all structural "
                    f"elements listed above — do not remove or summarize them."
                )
                retry_ops, _ = generate_delta(
                    target_path, target_content, strategy, constitution,
                    error_feedback=gate_fb, extra_vars=extra_vars,
                )
                if retry_ops is None:
                    return strategy, None, f"gate failed, no retry output: {violations_str}"
                delta = Delta(
                    generation=generation, strategy=strategy.name,
                    intent=strategy.description, ops=retry_ops,
                )
                d2_new, d2_err = delta.execute_all(file_contents)
                if d2_err:
                    return strategy, None, f"gate retry dry-run failed: {d2_err}"
                gate_failed2, violations2 = _gate_check(delta, d2_new)
                if gate_failed2:
                    return strategy, None, f"gate still failed after retry: {'; '.join(violations2)}"
        ...
        with ThreadPoolExecutor(max_workers=len(strategies_to_run)) as executor:
            ...
            for future in as_completed(futures):
                ...
                if delta is None:
                    ...
                    # Per-note memory: record invalid (skip when --strategy filter active to allow retry)
        ...
        if any(d.strategy == "condense" for d in deltas) and intra_overlaps:
            overlap_desc = "\n".join(
                ...
                for o in intra_overlaps[:3]
            )
            extra_context = (
                f"## Intra-file overlap context\n"
                f"Automated detection found these sections share significant content:\n"
                f"{overlap_desc}\n\n"
                f"When a candidate removes or condenses content from a later section "
                f"that is already covered in an earlier section, this is duplication "
                f"removal — not information loss. Evaluate whether the EARLIER section "
                f"already contains the removed content."
            )
        if any(d.strategy == "dedup" for d in deltas) and target_overlap:
            canonical_content = file_contents.get(target_overlap.canonical_path, "")
            # Detect Q&A context: blockquote lines ("> ...") near the overlap
            has_qa_context = any(
            ...
            if canonical_content:
                qa_warning = (
                    "\n\n**Q&A context**: This note contains blockquote questions (`> ...`). "
                    "Good dedup transforms the Q&A exchange into a clean standalone explanation: "
                    "removes the `> question` blockquote, removes the 'Your math is correct' "
                    "conversational framing, and expresses the knowledge as a direct section — "
                    "with the key numbers preserved — followed by a wikilink to the canonical. "
                    "ACCEPT if: the key insight and numbers are present in a clean form and "
                    "the Q&A scaffolding has been removed. "
                    "VETO if: the candidate simply deleted content and left only a wikilink "
                    "with no inline explanation of the key numbers."
                ) if has_qa_context else ""
                extra_context = (
        ...
        # ── IDENTITY CHECK ──
        identity_won = result.best_id == IDENTITY_ID or result.best_id == ""

        # ── POST-RANK RETRY (first identity win only) ──
        if identity_won and not post_rank_retried and result.per_judge_reasoning:
            post_rank_retried = True
            best_delta = max(deltas, key=lambda d: result.advantages.get(d.id, -999))
            strat_obj, evars = strategy_lookup.get(best_delta.strategy, (None, {}))
            if strat_obj:
                reasoning_lines = "\n".join(
                    f"- {jid}: {r}" for jid, r in result.per_judge_reasoning.items()
                )
                judge_fb = (
                    f"Judges ranked your changes below the original. Their reasoning:\n"
                    f"{reasoning_lines}\n\n"
                    f"Revise your approach to directly address these concerns. "
                    f"Make a more targeted, clearly-motivated change."
                )
                print(f"  Post-rank retry: {strat_obj.name} with judge feedback...", flush=True)
                evars.setdefault("note_list", all_note_list)
                evars.setdefault("line_count", str(target_lines))
                retry_ops, _ = generate_delta(
                    target_path, target_content, strat_obj, constitution,
                    error_feedback=judge_fb, extra_vars=evars,
                )
                if retry_ops:
                    retry_delta = Delta(
                        generation=generation, strategy=strat_obj.name,
                        intent=strat_obj.description, ops=retry_ops,
                    )
                    _, dry_err = retry_delta.execute_all(file_contents)
                    if not dry_err:
                        result = grpo_rank(
                            original_content=target_content,
                            deltas=[retry_delta],
                        ...
                        deltas = [retry_delta]
                        if result.best_id != IDENTITY_ID and result.best_id != "":
                            print(f"  Post-rank retry WON — proceeding to gates.", flush=True)
                            identity_won = False

        if identity_won:
            print(f"\n  IDENTITY WINS — no candidate beat current version.")
            for d in deltas:
                save_delta_ops(d)  # save losers for diagnosis
                adv = result.advantages.get(d.id, 0)
                diff_preview = d.render_for_ranking(file_contents)
                # Show first 500 chars of diff for diagnosis
                print(f"    [{d.strategy}] advantage={adv:.2f}, diff preview:")
                for line in diff_preview[:500].splitlines():
                    print(f"      {line}")
                if len(diff_preview) > 500:
                    print(f"      ... ({len(diff_preview)} chars total)")
            _record_losers(deltas, IDENTITY_ID, generation, target_path, result.advantages)
            # Per-note memory: count identity wins per strategy (skip after 3)
            for d in deltas:
                cache = note_tried.setdefault(target_path, {})
                prev = cache.get(d.strategy, 0)
                cache[d.strategy] = (prev + 1) if isinstance(prev, int) else 1
            history = load_history()
            save_generation_metadata(generation, {
                "generation": generation, "target": target_path,
        ...
        # Supermajority gate for shrinking strategies (densify, condense, dedup, normalize).
        # These strategies remove content; a simple Borda majority is insufficient because
        # judges are calibrated to reward surface-level conciseness and may not detect
        # load-bearing structural sentences (previews, signposting). Require ≥5/7 judges
        # to have ranked the winner above identity before any shrinking change is adopted.
        _SHRINKING_STRATEGIES = {"densify", "condense", "dedup", "normalize"}
        if winner.strategy in _SHRINKING_STRATEGIES and result.per_judge:
            n_judges = len(result.per_judge)
            beats_identity_count = sum(
                1 for jranks in result.per_judge.values()
                if jranks.get(winner.id, 999) < jranks.get(IDENTITY_ID, 999)
            )
            required = max(5, round(n_judges * 0.71))
            if beats_identity_count < required:
                print(f"\n  IDENTITY WINS — {winner.strategy} supermajority check: "
                      f"{beats_identity_count}/{n_judges} judges beat identity "
                      f"(required {required}).")
                _record_losers(deltas, IDENTITY_ID, generation, target_path, result.advantages)
                for d in deltas:
                    note_tried.setdefault(target_path, {})[d.strategy] = "below_threshold"
                save_generation_metadata(generation, {
                    ...
                    "outcome": "below_threshold",
                    "winner_advantage": winner_advantage,
                    "per_judge_rankings": result.per_judge,
                })
                continue
        ...
        # B3 fix: include create_file/rename_file paths so new files aren't flagged as broken links
        ...
        for op in winner.ops:
            ...
            if op.kind == "rename_file" and op.new_path not in all_note_paths:
                all_note_paths.append(op.new_path)
        ...
        if gate_failed:
            ...
            if not strategy_filter:
                ...
                # With strategy filter: allow a few retries, but cap at 3 vetoes
                cache = note_tried.setdefault(target_path, {})
                veto_key = f"{winner.strategy}_vetoes"
                prev = cache.get(veto_key, 0)
                cache[veto_key] = prev + 1
                if prev + 1 >= 3:
                    cache[winner.strategy] = "vetoed"
...
def main():
    ...
    parser.add_argument("--target", type=str, default=None, help="Only target notes matching this substring")
    parser.add_argument("--strategy", type=str, default=None, help="Only run this strategy (e.g. dedup, split)")
    args = parser.parse_args()
    run_evolution(max_gen=args.max_gen, group_size=args.group_size, push=not args.no_push,
                  target_filter=args.target, strategy_filter=args.strategy)
