...
### Optional: Interview Talking Points

Any note may optionally include a short "Interview Talking Points" section (10-20 lines)
...
### Reading Order and Domain Linearity

A domain's notes form a **linear reading order** when a reader can traverse
from note 1 to note N without encountering a forward reference to a concept
not yet defined in the sequence. A *forward reference* is a wikilink or
prerequisite pointing to a note later in the sequence where the linked concept
is required — not optional — for local comprehension.

**Prerequisite** (blocking): Note B cannot be understood without Note A.
Declare "Prerequisites: [[A]]" at the top. The reader must read A before B.

**Cross-link** (non-blocking): Note B references Note A for deeper context,
but B is locally comprehensible without A. Use inline wikilink or See Also.

Test: if removing [[A]] from B leaves an undefined term or unjustified claim,
it is a prerequisite. If removing it only reduces depth, it is a cross-link.

A cross-link to a later note in the sequence is permitted when:
1. It appears in See Also (non-blocking by convention), or
2. The inline reference includes a one-line summary sufficient for local
   comprehension

### Index notes

An **index note** (`index.md`) is navigation infrastructure. It lists notes
in reading sequence with one-line descriptions. It contains no original
explanations. It is not scored on content quality dimensions.

Index edits are evaluated on: completeness (all notes listed), ordering
(prerequisites before dependents), link validity (no dangling links).

Index edits and content edits must never appear in the same delta.

### See Also categories (recommended)

Notes with 5+ See Also entries should categorize them:
- **Foundations** — notes this one builds on (typically prerequisites)
- **Closely related** — sibling notes in the same concept cluster
- **Downstream** — notes that build on this one

Categorization is not required for notes with fewer than 5 See Also entries.
