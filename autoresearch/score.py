...
from shared import (
    ...
    has_required_sections, is_index_note,
)
from autoresearch_core.util import extract_json_object, find_paragraph_overlaps, detect_intra_overlaps
...
def score_uniqueness(note: Path, content: str, all_notes: list[Path],
    ...
    if not overlaps:
        cross_score = 10
        cross_result = {"score": 10, "reason": "No content overlaps detected", "suggestion": "Content uniqueness is excellent"}
    elif len(overlaps) == 1:
        cross_score = 5
        path, ratio, src_preview, tgt_preview = overlaps[0]
        cross_result = {"score": 5, "reason": f"Minor overlap with {path}",
        ...
        cross_result = {"score": 2, "reason": f"Overlaps found with {len(overlaps)} notes",
    ...
    if intra:
        n = len(intra)
        worst = intra[0]
        intra_score = 5 if n == 1 else 2
        intra_result = {"score": intra_score,
            "reason": f"{n} internal section overlap(s): {worst.section_b} restates {worst.section_a}",
            "suggestion": f"Condense '{worst.section_b}' — it restates '{worst.section_a}' ({worst.overlap_ratio:.0%} word overlap)"}
        if intra_score < cross_score:
            return intra_result
