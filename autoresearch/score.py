...
RATE_LIMIT_SECONDS = 10  # Min seconds between Claude CLI calls
...
def score_cross_linking(note: Path, content: str, all_notes: list[Path]) -> dict:
    ...
    for link in valid:
        target = REPO_ROOT / (link + ".md")
        if target.exists():
            target_content = read_note(target)
...
def score_uniqueness(note: Path, content: str, all_notes: list[Path]) -> dict:
    ...
    for other_note in all_notes:
        if other_note == note:
            continue
        other_content = read_note(other_note)
        for para in paragraphs:
            ...
            for i in range(len(other_sentences) - 2):
                ...
                if len(para_words) > 10 and len(chunk_words) > 10:
                    ...
                    if overlap > 0.7:
                        overlaps.append((relative_path(other_note), overlap))
                        break
            if overlaps:
                break
...
def score_note(note: Path, all_notes: list[Path], rule_only: bool = False, concurrency: int = 1) -> dict[str, dict]:
    ...
    # Rule-based dimensions (instant)
    scores["Cross-Linking"] = score_cross_linking(note, content, all_notes)
    scores["Code Quality"] = score_code_quality(content)
    scores["Uniqueness"] = score_uniqueness(note, content, all_notes)
    scores["Naming & Structure"] = score_naming_structure(note, content)
...
def main():
    ...
    if args.rule_only:
        ...
        print(f"(Claude CLI calls will be rate-limited to 1 per {RATE_LIMIT_SECONDS}s)\n")
