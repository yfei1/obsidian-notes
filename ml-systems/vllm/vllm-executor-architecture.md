...
vLLM's **executor** takes a `SchedulerOutput` from the engine, broadcasts it to every GPU worker, and collects results. Two backends: **MultiprocExecutor** uses shared memory + ZMQ (a lightweight messaging library) for dispatch; **RayDistributedExecutor** uses [[ml-systems/vllm/vllm-ray-compiled-graph|Ray Compiled Graph]]. Both talk to the exact same worker code — the difference is only in how the dispatch message travels.
...
This is the **control plane** problem — delivering instructions from scheduler to workers. It is separate from the **data plane** — GPU-to-GPU tensor transfers during the model forward. Data plane examples: NCCL allreduce (summing partial results across GPUs that share the same layers via [[ml-systems/distributed/tensor-parallelism|tensor parallelism]]) and NCCL send/recv (passing activations between pipeline stages — sequential groups of layers, each assigned to different GPUs via [[ml-systems/distributed/parallelism-strategies|pipeline parallelism]]). See [[ml-systems/distributed/vllm-distributed-groups]] for the data plane.
...
The executor handles only the top arrow. The bottom arrow (NCCL) is identical regardless of executor — it goes through [[ml-systems/distributed/vllm-distributed-groups|vLLM's distributed groups]].
...
Without async scheduling, dispatch (~100-300 us) sits on the critical path: the engine blocks until workers acknowledge receipt before the GPU can start, so the GPU idles between steps.

With **async scheduling** (`core.py:419-535`), the engine maintains a **batch queue** — a buffer of pre-dispatched batches. Because `enqueue()` returns as soon as the SHM write completes (~10-20 us) without waiting for worker acknowledgment, the engine can schedule and dispatch batch N+1 while the GPU is still executing batch N:
...
Batch queue size = `max_concurrent_batches`. Without [[ml-systems/distributed/parallelism-strategies|pipeline parallelism]], depth is 2: one batch executing on GPU, one being prepared. With PP=K, depth is `pp_size` — because PP keeps one micro-batch in flight per stage simultaneously, so the engine needs K batches pre-dispatched to keep all K stages busy at once. Fewer pre-dispatched batches than stages means at least one stage stalls waiting for input.

MultiprocExecutor achieves near-perfect overlap because `enqueue()` fires and returns in ~10-20 us. RayDistributedExecutor gets only partial overlap because Compiled Graph channel writes block inline for ~300 us–1 ms before returning — consuming most of the scheduling window that async scheduling is trying to reclaim.
...
- [[ml-systems/vllm/vllm-ray-compiled-graph]] — How Compiled Graph works, why it differs from regular Ray, and why it's being removed
- [[ml-systems/distributed/vllm-distributed-groups]] — The data plane: NCCL groups for TP, PP, EP
- [[ml-systems/distributed/parallelism-strategies]] — TP, PP, EP concepts
- [[ml-systems/distributed/tensor-parallelism]]
- [[ml-systems/vllm/vllm-weight-loading]] — executor initialization path that constructs the model and calls load_weights() before the serving loop starts
- [[ml-systems/vllm/vllm-ray-compiled-graph]] — Ray Compiled Graph as an alternative dispatch path to MultiprocExecutor's MessageQueue; being removed because async scheduling hides the latency CG was built to reduce
- [[ml-systems/inference/cuda-graph-inference-optimization]] — the executor's decode loop calls `graph.replay()` each step; CUDA graph capture happens during engine initialization before the executor begins serving requests
- [[ml-systems/inference/prefix-caching]] — prefix caching hooks into the BlockManager allocation path that the executor orchestrates
