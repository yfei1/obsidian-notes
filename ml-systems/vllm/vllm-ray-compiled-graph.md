...
**Every autoregressive decoding step dispatches a remote call to each GPU worker via Ray's scheduler — ~1-5 ms of RPC overhead per step, because every call re-enters Ray's dynamic task scheduler, serializes arguments through a central object store, and waits for an ObjectRef round-trip.** At 100 steps/second, that overhead idles the GPU 10–50% of the time on dispatch, not compute. Compiled Graph (CG) eliminates this by pre-wiring the call graph once at startup and replacing per-step RPCs with direct shared-memory writes (~300 µs) — because a fixed, pre-compiled channel skips Ray's dynamic task scheduler entirely on every step.

**CG's advantage is now obsolete.** Async scheduling (v0.15.0) overlaps CPU dispatch of batch N+1 with GPU execution of batch N — because GPU forward takes ~10-20 ms while dispatch takes ~150-500 µs, the entire dispatch window hides behind GPU compute. Once dispatch is hidden, CG's faster-dispatch benefit disappears while its structural costs remain: O(TP) sequential SHM writes (one per worker, because CG's per-edge channel model cannot broadcast) and a sync bubble (the CPU stall while waiting for a GPU NCCL receive to complete, ~5-15 µs at `ray_communicator.py:212`).

### How Ray Dispatch Works Without CG

A **Ray actor** is a worker process Ray manages — you call its methods via `actor.method.remote(data)`, which serializes arguments, routes them through a central scheduler and a shared-memory **object store** (Ray's inter-process data buffer), and returns an **ObjectRef** (a future the caller resolves with `ray.get()`). Each round trip pays that ~1-5 ms tax because every call re-enters the scheduler.

CG wires the call pattern once at startup, creating **fixed shared memory channels** — one per worker — that the driver writes to directly instead of routing through Ray's dynamic task scheduler.
...
Running example: **Llama 70B, PP=2, TP=4** (8 workers, 4 per pipeline stage — see [[ml-systems/distributed/parallelism-strategies]] for PP/TP concepts).

### DAG Construction (`ray_executor.py:547-640`)

The DAG does two things: (1) broadcast the SchedulerOutput to all TP=4 workers in stage 0, and (2) route each worker's **intermediate tensors** — the hidden-state output of stage 0 that stage 1 needs as input — to the matching stage-1 worker so the next set of layers can continue the computation.
...
**Most users never hit CG** because the Ray backend is not the default. The backend selection logic (`config/parallel.py:751-792`) defaults to `"mp"` (MultiprocExecutor) for both single-node and multi-node deployments — Ray adds process-management overhead that only pays off when Ray already owns the cluster. Concretely: `vllm serve model --tensor-parallel-size 4` uses MultiprocExecutor (SHM MessageQueue, no Ray, no CG).

Ray backend is selected when:

- `--distributed-executor-backend ray` is passed explicitly
- vLLM runs inside a Ray placement group (e.g., Ray Serve) — a **placement group** is a Ray construct that reserves and co-locates resources (CPUs, GPUs) for a set of actors; Ray Serve wraps every replica in one, triggering CG implicitly for all Ray Serve users
- `data_parallel_backend="ray"`

**When Ray is selected, CG is unconditional on the hot path.** `execute_model` always routes through the compiled DAG (`ray_executor.py:466-469`, built lazily at first step) — because every step needs low-latency dispatch and the DAG is already compiled, there is no per-step cost to using it. Cold-path operations (init, health checks, model loading) still use `ray.remote()` via `collective_rpc()` at `ray_executor.py:490-515` — they run infrequently enough that ~1-5 ms RPC overhead is irrelevant.

Three env vars tune CG behavior (only relevant when the Ray backend is active):

| Env var | Default | Effect |
|---------|---------|--------|
| `VLLM_USE_RAY_COMPILED_DAG_CHANNEL_TYPE` | auto | auto selects NCCL for inter-stage GPU tensors when available; forced to shm on TPU/XPU, causing GPU→CPU→SHM→CPU→GPU round-trips |
| `VLLM_USE_RAY_COMPILED_DAG_OVERLAP_COMM` | False | enables background communication overlap; off by default because it adds complexity without measured benefit |
| `VLLM_USE_RAY_WRAPPED_PP_COMM` | True | delegates PP NCCL to vLLM's existing PP group instead of CG's own channels, keeping the data plane consistent with MultiprocExecutor |

CG is only relevant when Ray manages the cluster — Ray Serve, RL frameworks like SkyRL, or explicit `--distributed-executor-backend ray`.
...
The v2 model runner explicitly skips CUDA graphs (a CUDA API feature that records a sequence of GPU kernel launches as a replayable graph, eliminating per-launch CPU overhead) for PP (`gpu/model_runner.py:523-529`): `"Skipping CUDA graph capture because pipeline parallel is enabled."` PP tensor transfer happens outside the graph boundary — through CG channels (Ray) or non-blocking `isend/irecv_tensor_dict` (multiproc) — so including it in a captured graph would break the transfer handshake.

TP allreduce/allgather within attention and MLP layers **are** captured (in PIECEWISE mode — a CUDA graph capture strategy that records individual kernel sequences as separate sub-graphs, allowing collective ops inside the forward pass to be included) because they occur inside the model forward that `CUDAGraphWrapper` wraps.

---

## Expert Parallelism Is Fully Independent of CG

EP (Expert Parallelism — routing tokens to specialized model sub-networks called experts, see [[ml-systems/distributed/parallelism-strategies]]) is independent of CG because all 8 EP backends (`all2all.py`) use `torch.distributed` process groups or custom libraries (DeepEP NVLink/RDMA, flashinfer NVLink, nixl RDMA, mori RDMA). `grep "compiled_dag|forward_dag"` across `vllm/distributed/` and `vllm/model_executor/layers/fused_moe/` returns zero matches. CG removal has zero EP impact.
...
Async scheduling (v0.15.0) overlaps CPU scheduling of step N+1 with GPU execution of step N — because GPU forward takes ~10-20 ms while dispatch takes ~150-500 µs, the scheduler hides dispatch latency entirely behind GPU compute. Once dispatch is hidden, CG's faster-dispatch advantage disappears, and its O(TP) write cost and sync bubble become pure overhead with no compensating benefit.

**The 10-15% Anyscale claim predates async scheduling** — back then `ray.remote()` overhead (~1-5 ms) was on the critical path; CG reduced it to ~300 µs. Async scheduling zeroes out this advantage.
...
**Why CG loses on TPOT (46 vs 41 ms)** *(same caveat)*: In steady-state decode, async scheduling's batch queue pays off because MultiprocExecutor dispatches batch N (~150 µs SHM write, fire-and-forget) and immediately schedules batch N+1 while the GPU runs. CG's `step()` is fully synchronous — schedule → CG dispatch (~500 µs O(TP) blocking writes) → wait for GPU → repeat — so the O(TP) write cost lands on the critical path every step.
...
- **Cross-node works**: MessageQueue's ZMQ TCP path (`shm_broadcast.py:409-426`) already handles remote workers — ZMQ (a messaging library) uses a PUB/SUB pattern where one publish fans out to all subscribers without blocking the sender, faster than CG's synchronous channel writes
- **Nothing lost**: `overlap_comm` disabled by default (`envs.py:58`); CG background thread unnecessary
- **Risk**: No cross-node PP benchmarks in PR #36836 (only TP=4 single-node on L4 GPUs). Multi-node PP regression is unlikely (same NCCL, faster dispatch) but unproven.
...
- [[ml-systems/vllm/vllm-executor-architecture]] — MultiprocExecutor, MessageQueue broadcast mechanism, and async scheduling that obsoletes CG's dispatch advantage
- [[ml-systems/distributed/parallelism-strategies]] — PP and TP concepts underlying the DAG topology (PP=2, TP=4 running example)
- [[ml-systems/distributed/vllm-distributed-groups]] — NCCL groups for TP, PP, EP that CG delegates to and that survive CG removal unchanged
- [[ml-systems/inference/cuda-graph-inference-optimization]] — CUDA graph capture and why PP tensor transfer is excluded
- [[ml-systems/distributed/tensor-parallelism]] — TP allreduce/allgather inside model forward runs through torch.distributed, independent of CG channels
...
Four independent reasons converge on removal:

1. **Async scheduling made dispatch speed irrelevant** — GPU execution (~10-20 ms) overlaps with next-batch scheduling (async scheduling: the engine schedules batch N+1 while the GPU executes batch N), hiding the entire dispatch window. Neither the ~100 µs MessageQueue write nor the ~500 µs CG O(TP) write loop lands on the critical path, so CG's faster-dispatch advantage over plain `ray.remote()` no longer translates into lower latency.
2. **CG is structurally slower** — O(TP) sequential channel writes vs O(1) shared memory broadcast, because CG's per-edge channel model requires one `SynchronousWriter.write()` call per worker rather than a single memcpy into a shared region. This is not a tunable parameter — it is a consequence of Ray's general-purpose DAG design where each edge carries independent data. PR #36836 confirms the cost: CG TPOT 45.88 ms vs MP 40.95 ms.
3. **PP tensor routing is trivially replaceable** — MultiprocExecutor uses `gpu_worker.isend/irecv_tensor_dict` (`gpu_worker.py:807-848`) with the same NCCL (NVIDIA's GPU collective communication library) as CG's `RayPPCommunicator`, so removing CG loses no data-plane capability. The NCCL groups survive unchanged; only the dispatch wrapper changes.
4. **Maintenance cost** — unresolved NCCL hangs across multiple Ray releases, because CG's `stream.synchronize()` stall (see `ray_communicator.py:212`) interacts poorly with Ray's actor lifecycle management in ways that plain `torch.distributed` NCCL calls avoid. Each Ray upgrade requires re-validating CG's synchronization behavior against the new actor scheduler.
...
- [[ml-systems/distributed/vllm-distributed-groups]] — NCCL groups for TP, PP, EP that CG delegates to and that survive CG removal unchanged
- [[ml-systems/distributed/parallelism-strategies]] — PP and TP concepts underlying the DAG topology (PP=2, TP=4 running example)
- [[ml-systems/inference/cuda-graph-inference-optimization]] — CUDA graph capture and why PP tensor transfer is excluded from captured graphs
- [[ml-systems/distributed/tensor-parallelism]] — TP allreduce/allgather inside model forward runs through torch.distributed, independent of CG channels
