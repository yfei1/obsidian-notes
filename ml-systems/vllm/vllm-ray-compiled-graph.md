...
**Every autoregressive decoding step dispatches a remote call to each GPU worker via Ray's scheduler — ~1-5 ms of RPC overhead per step, because every call re-enters Ray's dynamic task scheduler, serializes arguments through a central object store, and waits for an ObjectRef round-trip.** At 100 steps/second that overhead idles the GPU 10–50% of the time waiting for dispatch, not compute. Compiled Graph eliminates this by pre-wiring the call graph once at startup and replacing per-step RPCs with direct shared-memory writes (~300 µs) — because a fixed, pre-compiled channel skips Ray's dynamic task scheduler entirely on every step. CG's advantage is now obsolete: async scheduling (v0.15.0) overlaps dispatch with GPU execution, hiding the entire ~1-5 ms window behind ~10-20 ms GPU compute, so CG's faster-dispatch benefit disappears while its structural costs (O(TP) sequential writes, sync bubble — the CPU stall while waiting for a GPU NCCL receive to complete) remain.
