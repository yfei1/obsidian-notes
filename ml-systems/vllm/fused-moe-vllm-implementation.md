...
**Prerequisites**: [[ml-systems/foundations/mixture-of-experts]] (MoE architecture, router, expert dispatch), [[ml-systems/vllm/vllm-weight-loading]] (weight_loader convention), [[ml-systems/distributed/tensor-parallelism]] (TP rank, column-parallel vs replicated linear).

## Role in System

This note covers how vLLM implements MoE layers in practice: why the router uses `ReplicatedLinear`, how `FusedMoE` separates routing scores from data transformation, the fused weight buffer layout (`w13_weight` / `w2_weight`), and the per-expert weight loading loop. The conceptual MoE mechanism (routing, expert FFNs, parameter counts) is in [[ml-systems/foundations/mixture-of-experts]].
...
Access via `getattr(param, "weight_loader", default_weight_loader)`. All vLLM parallel layers (QKVParallelLinear, ColumnParallelLinear, etc.) use the same pattern — see [[ml-systems/vllm/vllm-weight-loading]] for the full convention.
...
- [[ml-systems/foundations/mixture-of-experts]] — MoE architecture, routing mechanism, parameter counts, FusedMoE kernel concept
- [[ml-systems/foundations/pt-moe-architecture]] — the model architecture this note implements
- [[ml-systems/vllm/vllm-weight-loading]] — weight_loader convention used by all vLLM parallel layers
- [[ml-systems/vllm/pt-moe-vllm-implementation]] — PT-MoE vLLM implementation using FusedMoE + ReplicatedLinear
- [[ml-systems/distributed/tensor-parallelism]] — TP rank sharding mechanics; why ColumnParallelLinear breaks router consistency
- [[ml-systems/distributed/parallelism-strategies]] — broader parallelism taxonomy
- [[ml-systems/vllm/vllm-weight-loading]] — gotchas when calling FusedMoE.weight_loader (weight_name guard, transpose requirement, expert iteration order)
