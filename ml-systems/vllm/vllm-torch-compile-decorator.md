...
**Compile overhead**: Compilation runs once during warmup before vLLM serves traffic, so no live request absorbs the latency. The tradeoff is worthwhile only when the model is graph-break-free — a fragmented graph produces many small compiled pieces, each with its own Dynamo + Inductor cost, and that additive compilation time can exceed the fusion benefit from the resulting pieces. More unique input shapes also multiply cost: each shape that misses the compiled-graph cache triggers a full retrace because Dynamo re-specializes from scratch.

**Piecewise compilation**: A single compiled graph is ideal because Inductor can fuse across the entire forward pass. When graph breaks are unavoidable — e.g., `all_reduce` (a cross-GPU collective that synchronizes tensors) in PT-MoE's cross-track communication — Dynamo splits at each break and compiles each piece separately. Each piece still gets fusion within its boundaries, but runtime cost rises because each piece is a separate GPU launch: CPU dispatch overhead accumulates once per piece per step rather than once per full forward pass.

**`enable_if` for conditional compilation**: Some submodules are only compile-safe under specific configs, but a class-level decorator can't inspect runtime config. The `enable_if` callback solves this — it receives `VllmConfig` and returns a bool. When it returns `False`, the wrapped `__init__` sets `do_not_compile = True` and skips the counter increment. Skipping the increment is what matters: vLLM's detection path treats a non-incrementing counter as "no compiled model present" and falls back to eager — so `enable_if` is a clean per-class override without requiring changes to the detection logic.
