...
**Scope**: Implementation walkthrough for integrating [[ml-systems/foundations/pt-moe-architecture]] into vLLM — process group setup, module design, and performance pitfalls.

**Prerequisites**: [[ml-systems/foundations/pt-moe-architecture]] (track parallelism concept), [[ml-systems/distributed/parallelism-strategies]] (TP/EP fundamentals), [[ml-systems/distributed/vllm-distributed-groups]] (vLLM process group internals).
...
**PT-MoE runs 8 independent model copies (tracks) in parallel, so TP all-reduces must stay within each track's 4-GPU subset — but vLLM's TP group is global by default.** In a 32-GPU deployment (PT=8 tracks × TP=4 GPUs/track), naively dropping PT-MoE into vLLM makes every TP all-reduce cross all 32 GPUs: wrong results (weights are sharded per-track, so cross-track reduction produces garbage) and 8x the communication volume.

The fix requires exactly two changes. First, override `_TP` — vLLM's global variable (parallel_state.py:1213) that every layer reads via `get_tp_group()` — with the intra-track group; this scopes all existing vLLM layers to the correct 4-GPU subset without any per-layer changes. Second, add one custom module, `PTSegment`, to handle the cross-track mean all-reduce at segment boundaries (every 4 layers): all-reduce sums activations across all 8 tracks, then divides by track count to keep activation magnitudes stable. Everything else — attention, FFN, MoE — reuses standard vLLM components unchanged.
...
PT is **above** TP and EP. TP and EP are **within** each track. They are orthogonal (each dimension can be set independently) and composable. See [[ml-systems/distributed/parallelism-strategies]] for TP/EP fundamentals.
...
- **Sliding window**: `Attention(per_layer_sliding_window=config.sliding_window)` — pass raw value, no +1 (verified against Gemma2, Mistral, AJAX). See [[ml-systems/vllm/vllm-weight-loading]] gotcha #7.
...
Narrowing `_TP` after `initialize_model_parallel()` was validated safe for PT-MoE's current models (V9/V11, 1 KV head). See [[ml-systems/distributed/vllm-distributed-groups]] for how the rebuild works.
...
| Config divergence | Monitor | `parallel_config.tensor_parallel_size` stays 32, `get_tp_group().world_size` is 4. See note below. |

**Config divergence detail**: SP (sequence parallelism — splitting a sequence across TP ranks to reduce per-rank activation memory) padding at gpu_model_runner.py:2979 reads `parallel_config.tensor_parallel_size` (32) rather than the actual group size (4), because the config was never updated to reflect the narrowed group. At seq=512: pads to next multiple of 32 = 512, same as next multiple of 4 = 512 — no waste. At seq=481: pads to 512 (31 wasted slots, ~6%) instead of 484 (3 wasted slots, ~0.6%). Wasteful but correct.

### Blast radius for future parallelism support

PP and DP sit *above* TP in the hierarchy, so they are unaffected by narrowing `_TP` — adding PP (pipeline parallelism — splitting layers across GPUs) or DP (data parallelism — splitting the batch across GPUs) requires zero changes to the rebuild pattern. PCP/DCP (internal framework terms for finer-grained PP/DP variants) do require the same rebuild treatment because they introduce their own communication groups at the TP level. EP should also be rebuilt to scope within-track; currently safe because V9/V11 have 1 KV head, but incorrect for models with more KV heads.

### Head inflation is fragile — use truthful config + validation patching instead

vLLM validates `num_heads % tp_size == 0` at config construction. The original workaround inflated `num_attention_heads = per_track_heads * num_tracks` to satisfy this check against the global `tp_size=32`. This breaks because Pydantic re-creates ModelConfig during VllmConfig construction — the copy drops computed fields, losing the inflation silently. See [[ml-systems/distributed/vllm-distributed-groups]] "Pydantic config copy pitfall". The correct approach: report truthful per-track values and monkey-patch 3 methods in the plugin (`verify_with_parallel_config`, `get_num_attention_heads`, `get_num_kv_heads`) to substitute `within_track_tp = tp // num_tracks` wherever the global TP size would be divided into head counts.

### CUDA graphs require `_PT` in graph capture context

`--enforce-eager` is currently required because vLLM's `graph_capture()` (parallel_state.py:1296) only wraps `_TP` and `_PP` — it has no knowledge of `_PT`. CUDA graphs record all GPU ops issued on a single CUDA stream and replay that recording verbatim. The `_PT.all_reduce()` in `PTSegment.forward()` is a NCCL collective, which executes on NCCL's own internal stream rather than the capture stream — so the collective is invisible to the recording and crashes graph capture at replay. Fix: monkey-patch `graph_capture` to also enter `_PT.graph_capture(context)`, mirroring how `_TP` is handled. Expected gain: 1.5-2x decode throughput.
...
Everything else uses standard vLLM components. PTSegment is the only new logic — D decoder layers followed by a single cross-track mean all-reduce:
...
### How `_PT` routes each rank to the correct cross-track group

Each rank must call `all_reduce` on the group containing only its TP-sibling ranks across tracks — rank 0 on `[0,4,8,...]`, rank 1 on `[1,5,9,...]`, etc. This scoping happens automatically because of how vLLM initializes groups.

NCCL (NVIDIA's GPU communication library) requires every rank to call `new_group()` for every group, even groups it won't join — because NCCL uses the call sequence to establish a shared communication context across all participants. So when `init_model_parallel_group` receives `[[0,4,8,...], [1,5,9,...], ...]`, it calls `torch.distributed.new_group()` for every list on every rank. The resulting `GroupCoordinator` — vLLM's wrapper around a `torch.distributed` process group that adds helpers like `all_reduce` — stores only the group whose rank list contains the current rank (parallel_state.py:316-395). Rank 0's `_PT.device_group` therefore points to `[0,4,8,...]`; rank 1's to `[1,5,9,...]`. When `PTSegment` calls `_PT.all_reduce()`, each rank automatically communicates with its correct cross-track peers. See [[ml-systems/distributed/vllm-distributed-groups]] for the full `new_group` loop.

`all_reduce` is **out-of-place** — it returns a new tensor rather than mutating in place (parallel_state.py:489-511), because PyTorch custom ops cannot both mutate and return a new tensor in the same op.

### Mean vs sum: use mean

A plain sum scales activation magnitude by track count, destabilizing downstream layers — so the merge must divide by `num_tracks`. The PT paper's Algorithm 1 shows `h = all-reduce(h_0, ..., h_{N-1})`, which reads as a plain sum. The internal training framework (ajax — internal JAX-based trainer) uses `jnp.mean(x, axis=0)` (parallel_track_transformer.py `MeanOutputMerger`). The training code is authoritative — use mean.
...
- [[ml-systems/foundations/pt-moe-architecture]] — Architecture details, layer patterns, checkpoint structure
- [[ml-systems/distributed/parallelism-strategies]] — TP, EP, PP fundamentals
- [[ml-systems/foundations/mixture-of-experts]] — MoE internals, FusedMoE, load balancing
- [[ml-systems/distributed/vllm-distributed-groups]] — vLLM process group internals, rebuild pattern, config divergence risks
...
- [[ml-systems/gpu/python-import-binding]] — import binding gotcha for the `_PT` graph_capture monkey-patch
...
- [[ml-systems/vllm/vllm-weight-loading]] — step-by-step walkthrough of the multi-track weight loading pattern PT-MoE uses
- [[ml-systems/distributed/vllm-process-group-rebuild]]
- [[ml-systems/gpu/pt-moe-4norm-fusion-deep-research]] — deep research transcript analyzing kernel fusion opportunities for the 4-norm residual pattern in `afm_pt_moe.py`
