...
PT-MoE 150B is a **multi-track model** — each **track** is an independent set of transformer layers that shares the same input embedding but produces a distinct output distribution. 8 tracks train jointly; because a separate checkpoint per track would multiply storage and complicate deployment, the export pipeline concatenates per-track weights along dim 0 into a single checkpoint file — one checkpoint serves all 8 tracks.

This creates two problems at load time that the standard `load_weights` loop doesn't handle:

1. **Shape mismatch**: each GPU owns one track, so `gate_proj` expects `[2048, 5888]`; the checkpoint tensor is `[8, 2048, 5888]`. Passing the full tensor to `ColumnParallelLinear.weight_loader` causes it to slice across the track axis instead of the output axis — dim 0 = 8 is divisible by TP=4, so the slice succeeds dimensionally but writes wrong rows into each rank. Silent corruption.

2. **Storage order mismatch**: the 150B export pipeline stores linear weights as `[in, out]`; PyTorch's `nn.Linear` expects `[out, in]` (matching `y = xW^T`, where rows correspond to output neurons and are read sequentially in row-major memory). The 3B checkpoint is already `[out, in]`, so no transpose is needed there.

Shared weights (embedding, output norm) have no track dimension, so a blanket dim-0 slice would corrupt them — the fix must be selective.

### Checkpoint Shape Convention

The export pipeline stores every per-track tensor with `num_tracks` (8) as dim 0, and every shared tensor with a different leading dimension (e.g., `153600` for vocab). This convention makes `tensor.shape[0] == num_tracks` a reliable discriminator without inspecting tensor names:
...
After slicing dim 0, the tensor is shape-equivalent to a single-track model — `weight_loader` dispatch and TP sharding proceed without modification.

### The Extra Step: Slice, Then Delegate

The 150B `load_weights` inserts two steps before delegating to `weight_loader`: extract this rank's track slice (dim 0), then transpose 2D linears from `[in, out]` to `[out, in]`.

```python
track_idx = get_track_idx()  # _PT.rank_in_group: which of the 8 tracks this GPU owns (see [[ml-systems/distributed/vllm-distributed-groups]])
...
After track slicing, expert weights are 3D: `[300, 2048, 768]` — each expert slice is dispatched to `FusedMoE.weight_loader` with `shard_id` and `expert_id` to land at the correct offset in the fused `w13`/`w2` buffers. Full layout, dispatch logic, and the per-expert loop: [[ml-systems/vllm/fused-moe-vllm-implementation]].
...
- [[ml-systems/foundations/transformer-model-internals]] — the module hierarchy that `named_parameters()` walks
- [[ml-systems/distributed/parallelism-strategies]] — tensor parallelism and why weight_loader exists
- [[ml-systems/distributed/tensor-parallelism]] — ColumnParallelLinear / RowParallelLinear weight_loader shard logic
- [[ml-systems/inference/llm-inference-engines]] — where `load_weights()` fits in the engine initialization
- [[ml-systems/distributed/vllm-distributed-groups]] — GroupCoordinator fields (`rank_in_group` used for track index)
- [[ml-systems/vllm/pt-moe-vllm-implementation]] — the PT-MoE model that uses this loading pattern
- [[ml-systems/foundations/pt-moe-architecture]] — multi-track architecture that drives the dim-0 slicing convention
- [[ml-systems/foundations/mixture-of-experts]] — FusedMoE weight_loader shard_id mapping
- [[ml-systems/vllm/fused-moe-vllm-implementation]] — FusedMoE.weight_loader internals (shard_id, expert_id, w13/w2 layout)
- [[ml-systems/foundations/rotary-position-embedding]] — RoPE vs Global NoPE distinction that produces divergent checkpoint names
- [[ml-systems/vllm/vllm-model-integration]] — how vLLM model classes register weight_loader on parameters during __init__
- [[ml-systems/vllm/vllm-executor-architecture]] — executor initialization path that calls load_weights() before serving begins
