...
SP reduces redundant copies but does not reduce the peak attention memory on any single GPU. Attention requires each GPU to compute queries against keys and values for every token in the full sequence — even with SP, a GPU processing its sequence slice must access KV state for all other tokens to compute attention correctly. At 128K tokens with hidden dim 4096 in fp16, that KV state is 128000 × 4096 × 2 × 2 B = 2.0 GB per layer <!-- verify: 128000 * 4096 * 2 * 2 / 1024**3 < 2.1 -->, and across 32 layers that is 64 GB <!-- verify: 128000 * 4096 * 2 * 2 * 32 / 1024**3 < 65 --> — leaving little room on an 80 GB A100 <!-- source: A100 datasheet --> before weights or activations are counted.
