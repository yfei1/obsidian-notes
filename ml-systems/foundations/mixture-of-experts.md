...
**Prerequisites**: [[ml-systems/foundations/transformer-model-internals]] (dense FFN / SwiGLU), [[ml-systems/distributed/parallelism-strategies]] (tensor parallelism, all-reduce), [[ml-systems/vllm/vllm-weight-loading]] (weight_loader convention).
...
Everything outside the FFN — RMSNorm, residual connections, RoPE, QKV projections, o_proj — stays identical. For dense FFN internals (SwiGLU, gate/up/down projections), see [[ml-systems/foundations/transformer-model-internals]].
...
Full implementation details — buffer shapes, shard_id mapping, per-expert loading loop, and the weight_loader monkey-patch convention — are in [[ml-systems/vllm/fused-moe-vllm-implementation]].
...
- [[ml-systems/vllm/pt-moe-vllm-implementation]] — PT-MoE vLLM implementation using FusedMoE + ReplicatedLinear
- [[ml-systems/foundations/attention-mechanics]] — attention is unchanged by MoE
- [[ml-systems/vllm/vllm-weight-loading]] — weight_loader convention, checkpoint name remapping
- [[ml-systems/distributed/validating-parallelism-at-scale]] — verifying correctness of TP/EP routing under MoE parallelism
- [[ml-systems/foundations/parallel-track-architecture]]
- [[ml-systems/gpu/gpu-kernel-stack]] — how FusedMoE (custom op) interacts with torch.compile and Inductor fusion
- [[ml-systems/inference/flashinfer-vllm-integration]]
- [[ml-systems/inference/lora-vllm-serving]]
- [[ml-systems/vllm/vllm-weight-loading]] — expert weight loading via FusedMoE.weight_loader: shard_id mapping, per-expert iteration, w13/w2 fused buffer layout
