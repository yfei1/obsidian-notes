...
**Scope**: What LoRA is, why it works, the forward-pass math, adapter file format, and alpha scaling. Does not cover serving infrastructure or framework integration — see [[ml-systems/inference/lora-vllm-serving]] for multi-tenant serving in vLLM.

**Prerequisites**: [[ml-systems/foundations/transformer-model-internals]] (linear layers, QKV projections), [[ml-systems/foundations/attention-mechanics]] (attention structure).
...
- [[ml-systems/inference/lora-vllm-serving]] — multi-tenant LoRA serving: SupportsLoRA contract, Punica routing, adapter slot management
- [[ml-systems/foundations/transformer-model-internals]] — linear layer building blocks (QKV, FFN)
- [[ml-systems/vllm/vllm-model-integration]] — model registration and weight loading contract
