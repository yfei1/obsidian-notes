...
The kernel alone isn't sufficient — three surrounding constraints must hold. Each is an independent path through which HBM writes re-enter the pipeline and defeat the fusion.

**Merged weight storage** (packing `w_pre` and `w_post` into a single contiguous buffer so both load in one HBM transaction): Steps 2 and 4 each read a norm weight vector (`[8192]` bf16 = 16 KB) from HBM. If `w_pre` and `w_post` sit at non-contiguous addresses, the kernel issues two separate 16 KB HBM reads with a stall between them — the all-reduced tensor must wait in registers while each weight loads, breaking the SRAM-resident chain. Packing them into `[w_pre | w_post]` (32 KB contiguous) lets one coalesced transaction (a contiguous HBM read serviced in a single round-trip) load both before the norm begins — the same pattern `MergedColumnParallelLinear` uses for `gate_proj` and `up_proj`.

**Custom weight loading** (explicit slot routing so each checkpoint tensor lands in the correct position within the merged buffer): Merged storage requires correct slot assignment at load time. The checkpoint stores `attn_pre_residual_norm.weight` and `attn_post_norm.weight` as separate tensors; without explicit routing, `load_weights()` places them in the wrong merged-buffer slots — step 2 silently reads `w_post` weights instead of `w_pre`, producing incorrect norms with no error. The fix routes each tensor into its correct shard (`w_pre` → shard 0, `w_post` → shard 1), identical to `stacked_params_mapping` in `afm_pt_moe.py:443-447`.

**Custom op registration** (exposing the fused kernel to `torch.compile` so the Inductor graph treats it as a single atomic op rather than decomposing it): Even with a correct kernel and weights, `torch.compile` rewrites the compute graph via Inductor pattern matchers — without registration, the compiler does not recognize the fused op and decomposes it back into four separate ops, reintroducing HBM writes between them. No existing FlashInfer pattern covers `all_reduce -> rms_norm -> add -> rms_norm` (only `kARResidualRMSNorm` variants exist), so a new class `AllReducePostLNPattern` must be registered at `allreduce_rms_fusion.py:824-835` via `direct_register_custom_op()`.
