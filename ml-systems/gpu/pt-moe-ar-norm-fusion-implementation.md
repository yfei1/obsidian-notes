...
Three independent failure paths each break the invariant at a different layer of the stack. Each has a distinct fix.

**Failure 1 — non-contiguous norm weights, layer: memory layout.** Steps 2 and 4 each load a norm weight vector (`[8192]` bf16 = 16 KB) from HBM. Non-contiguous `w_pre` and `w_post` force two separate 16 KB HBM reads — because the GPU cannot coalesce (combine into a single transaction) non-adjacent addresses, the all-reduced tensor stalls in registers between loads, breaking the SRAM-resident chain. Fix: **merged weight storage** — pack `w_pre` and `w_post` into a single contiguous `[w_pre | w_post]` buffer (32 KB) so one coalesced HBM transaction loads both before either norm begins. Same pattern as `MergedColumnParallelLinear` for `gate_proj`/`up_proj`.

**Failure 2 — default weight loading, layer: checkpoint routing.** The checkpoint stores `attn_pre_residual_norm.weight` and `attn_post_norm.weight` as separate tensors. `load_weights()` has no knowledge of the merged buffer layout — without explicit routing it places tensors arbitrarily, so step 2 silently reads `w_post` weights instead of `w_pre`, producing wrong norms with no runtime error. Fix: **custom weight loading** — route each tensor into its correct shard (`w_pre` → shard 0, `w_post` → shard 1) explicitly, using the same `stacked_params_mapping` convention (vLLM's mechanism for routing checkpoint tensors into pre-allocated merged weight buffers) as `afm_pt_moe.py:443-447`.

**Failure 3 — `torch.compile` graph rewrite, layer: compiler.** Even with correct kernel and weights, Inductor's pattern matchers rewrite the compute graph at compile time. Without registration, the compiler does not recognize the fused op as a single node — because it sees four separate Python ops, it decomposes them back, reintroducing HBM writes between steps. No existing FlashInfer pattern covers `all_reduce -> rms_norm -> add -> rms_norm` (only `kARResidualRMSNorm` variants exist). Fix: **custom op registration** — register `AllReducePostLNPattern` at `allreduce_rms_fusion.py:824-835` via `direct_register_custom_op()` (a PyTorch mechanism that exposes a hand-written kernel as a single atomic node in the `torch.compile` graph) so Inductor treats the entire four-step sequence as one node.
