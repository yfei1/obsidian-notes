...
4. **[[ml-systems/gpu/pt-moe-4norm-fused-kernel-integration]]** — Integration path. CustomOp tier selection, torch.compile interaction, and hybrid implementation plan. Tier selection requires note 1's kernel interface; the decision to prioritize decode-path integration requires the ROI framing from notes 2–3.
5. **[[ml-systems/gpu/pt-moe-4norm-tp-fusion-opportunity]]** — TP extension. Under tensor parallelism, each rank independently applies the post-norm — executing it redundantly. Fusing the AllReduce with the norm eliminates this redundancy. Extends the CustomOp pattern established in note 4; the AR+norm opportunity only exists once the single-rank fusion from note 4 is in place.
