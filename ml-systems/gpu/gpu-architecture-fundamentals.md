...
- **Hierarchical Sizing & Warp Independence**: To prevent register spills and preserve multi-block SM occupancy, tiling is structured hierarchically: Block Tile (sized in Shared Memory) $\to$ Warp Tile (matrix instruction level) $\to$ Thread Tile (small register accumulator tiles). Because warps within a block compute disjoint, independent Warp Tiles (e.g. 8 warps in a 256-thread block dividing a $128 \times 128$ tile into eight $64 \times 32$ sub-tiles), they share zero data dependencies—enabling the warp scheduler to execute Warp 1's compute while Warp 0 stalls on memory loads.
