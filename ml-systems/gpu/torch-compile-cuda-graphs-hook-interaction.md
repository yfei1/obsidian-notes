...
Prerequisite: [[ml-systems/gpu/pytorch-module-hooks]] — the `__call__` → `_call_impl` → hooks → `forward()` call chain.
...
- [[ml-systems/gpu/pytorch-module-hooks]] — the `__call__` → `_call_impl` → hooks → `forward()` call chain this note builds on; also covers hook registration and why hooks run during CUDA graph capture but not replay
- [[ml-systems/gpu/torch-compile-graph-breaks]] — empirical test results: what patterns break `fullgraph=True` vs compile fine
- [[ml-systems/gpu/gpu-memory-hierarchy]] — GPU memory and compute context for kernel fusion trade-offs
- [[ml-systems/inference/llm-inference-engines]] — nano-vllm context: where these mechanisms appear in a real inference stack
- [[ml-systems/gpu/gpu-kernel-stack]] — Triton, Inductor, CUDA graphs composition with experiments and generated code
- [[ml-systems/vllm/vllm-torch-compile-decorator]] — vLLM's `@support_torch_compile` decorator: opt-in contract, inner vs outer class placement
- [[ml-systems/inference/cuda-graph-inference-optimization]]
- [[ml-systems/gpu/nsight-systems-profiling]] — Nsight Systems timeline for diagnosing CUDA Graph replay overhead vs per-kernel launch overhead
