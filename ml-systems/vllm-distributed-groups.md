...
- **EP** — expert parallel: ranks holding different expert sub-networks in a **MoE (Mixture-of-Experts)** layer. A MoE layer replaces the standard feed-forward sub-layer with many specialist sub-networks called **experts** — each a small independent feed-forward network. A learned **router** — a small linear layer trained alongside the model — selects exactly one expert per token. Because different tokens route to different experts and each expert lives on a different GPU, routing a token to its expert may require a cross-GPU transfer before that token's forward pass can proceed. EP is the communicator group scoped to all ranks that hold experts for a given model replica.
- **DCP** — disaggregated-context parallel: ranks that move KV cache entries between GPU pools. The **KV cache** stores the key/value tensors computed for prior tokens so they need not be recomputed on each step (see [[ml-systems/kv-cache-internals]]). **Disaggregated serving** splits generation into two phases — **prefill** (one forward pass over the full prompt, producing all token representations) and **decode** (one step per output token) — and runs each phase on a separate GPU pool on separate machines. Because neither pool can read the other pool's GPU memory directly, the KV cache entries produced during prefill must be explicitly copied to the decode pool before decode can begin. DCP is the communicator group that performs that transfer.
...
Distributed inference requires different communication patterns for different parallelism axes. A **pipeline stage** is a contiguous slice of the model's layers assigned to one GPU in a pipeline-parallel setup; TP ranks within a stage share one layer's weight shards. A **weight-parallel operation** is a matrix multiply whose weight matrix is split across GPUs, so each GPU computes only a partial dot product. Those partial results must be summed before the next layer can proceed. TP ranks do this with **all-reduce**: every rank sends its partial result, the values are summed element-wise across all ranks, and each rank receives the full result. In the 32-GPU running example (hidden=8192, seq=512), each GPU holds a `[8192, 256]` column slice (8192÷32=256 columns) and computes a partial dot product `[512, 8192] × [8192, 256] → [512, 256]`; those 32 partial results are all-reduced to produce the full `[512, 8192]` output (8,388,608 bytes bf16: `512×8192×2`) before the next layer proceeds. PP ranks send activations forward from one stage to the next; DP ranks synchronize gradients across **replicas** (identical copies of the model running on separate GPU sets, each processing different input batches).
...
`torch.distributed.new_group` is a **collective barrier** — every rank in the entire job must call it before any rank proceeds, even ranks that do not belong to the group being created. NCCL's communicator setup requires a globally synchronized handshake: every participant exchanges device topology information (NVLink graph, PCIe topology) and staging buffer addresses. Each rank needs that topology to route collective operations to the right GPUs. Because the handshake waits for every rank to check in, skipping the call on even one rank stalls all others indefinitely. The constructor (parallel_state.py:316-395) therefore iterates ALL group rank lists, calling `new_group` twice per group (once for NCCL, once for Gloo), and stores only the group the current rank belongs to. For TP=32 that is 1 group × 2 backends = 2 calls per rank; after the PT-MoE rebuild into 8 tracks × TP=4 it is 8 groups × 2 backends = 16 calls per rank.
...
- **`local_rank`**: physical GPU slot on this node, set by `torchrun` — PyTorch's distributed launcher, which starts one process per GPU and assigns each a node-local index counting from 0. Rank 12 on a node covering GPUs 8–15 has `local_rank = 12 - 8 = 4`
...
All three attributes are scoped differently — job, node, group. Using the wrong one produces a silent failure: the right type of index, but counting from the wrong origin.

**Weight sharding** uses `rank_in_group` from the TP group because weight shards are indexed by position within the TP group: shard 0 goes to the first rank in the group, not the first rank on the node. `local_rank` answers "which GPU slot on this machine?" — it counts from the node boundary, not the group boundary. Rank 12 has `local_rank=4` but `rank_in_group=0` in TP group [12,13,14,15]; using `local_rank` would load the fifth shard instead of the first, silently wrong on every rank outside the first TP group.

**PT-MoE track assignment** also needs `rank_in_group`, but from a different group — the **cross-track group** (defined below) — rather than the TP group. After rebuilding into 8 tracks × TP=4, each track runs an independent forward pass on different inputs. The rebuilt TP group encodes which 4 GPUs share a forward pass, but not which track number (0–7) a rank belongs to across the full fleet. `rank` is job-scoped and `local_rank` is node-scoped — neither directly encodes which of the 8 tracks a rank belongs to.

PT-MoE resolves this by constructing a **cross-track group** per **slot index** — where slot index is a rank's position within its own track's rank list (0–3 for TP=4). Each track has exactly one rank at each slot index, so collecting all slot-0 ranks across 8 tracks yields one rank per track: [0,4,8,12,16,20,24,28]. Rank 12 sits at slot 0 of track 3 (12 % 4 == 0), making it the fourth entry in this cross-track group — `rank_in_group=3`. PT-MoE uses that value as the track index because it counts track membership across the full 32-GPU fleet, independent of node boundaries.
...
The default `_TP` group spans all 32 GPUs, but PT-MoE runs 8 independent model replicas (tracks) in parallel, each processing a different input. An all-reduce over all 32 GPUs sums activations from different inputs — producing garbage at every layer — so PT-MoE needs all-reduce scoped to 4 GPUs per track, touching only the ranks holding that track's activations.

Destroy `_TP` and replace it with a narrower coordinator covering only the 4 ranks within one track. Call `.destroy()` on the old coordinator first — a leaked NCCL communicator holds GPU memory and internal threads, causing OOM or deadlocks on subsequent collectives. Then assign a new `GroupCoordinator` — built with the 8 per-track rank lists — to the module-level global `ps._TP` (where `ps` is `import vllm.distributed.parallel_state as ps`). `ps._TP` is a plain Python reference, so reassigning it immediately redirects all subsequent `get_tp_group()` calls to the new 4-GPU communicator.

**Timing constraint:** The rebuild must happen inside `model.__init__()`, before any layers are constructed. Layers capture `get_tp_group()` at construction time, so a rebuild after layer init leaves those layers holding a stale coordinator pointing to the old 32-GPU group — their all-reduces still span all 32 GPUs.
