...
- **DCP** — disaggregated-context parallel: ranks partitioning the KV cache when prefill and decode run on separate GPU pools. **Disaggregated prefill/decode** is a serving topology where prefill (computing the first token from a prompt) runs on one GPU pool and decode (generating each subsequent token) runs on another; their KV caches must be explicitly synchronized across the split.
