...
- **EP** — expert parallel: ranks holding different **expert sub-networks** in a MoE (Mixture-of-Experts) layer. In a MoE layer, each input token is routed to one specialist feed-forward network rather than passing through one shared network; EP assigns each specialist to a different GPU.
- **DCP** — disaggregated-context parallel: ranks partitioning the KV cache across a prefill/decode split. In **disaggregated prefill/decode** serving, prefill (computing the first token from a prompt) and decode (generating each subsequent token) run on separate GPU pools — so their KV caches must be explicitly synchronized across the split; DCP is the communicator group that performs that synchronization.
...
Each pattern needs its own **NCCL communicator** — a named handle to a fixed set of GPUs that can issue collectives among themselves — scoped to exactly the right ranks. A communicator built over the wrong rank set either silently skips GPUs or deadlocks waiting for ranks that never participate. **vLLM solves this with one 5D rank tensor (`ExternalDP × DP × PP × PCP × TP`) from which all six groups are derived by transpose + reshape** — same data, different views, each producing the correct rank lists without redundant bookkeeping.
...
PT-MoE solves this by defining a **slot index** — a rank's position within its own track's rank list (0–3 for TP=4). Each slot value appears exactly once per track, so every rank has a unique (track, slot) pair. PT-MoE then forms a **cross-track group** for each slot value by collecting the rank at that slot from every track: slot 0 of every track forms the group [0,4,8,12,16,20,24,28]. Rank 12 sits at slot 0 of track 3, so it is the fourth entry in this cross-track group → `rank_in_group=3`. PT-MoE uses this `rank_in_group` value as the **track index** because it counts exactly which track a rank belongs to across the full 32-GPU fleet, independent of node boundaries.
