...
vLLM builds six process groups from one 5D rank tensor with layout `ExternalDP × DP × PP × PCP × TP`. In distributed training, each process is assigned a **rank** — a unique integer index across the entire job, from 0 to `world_size − 1`. A **collective** is an operation — like all-reduce or broadcast — that every rank in a group executes together; all-reduce sums a tensor across all ranks and gives every rank the result. Each tensor axis corresponds to one parallelism dimension:
...
- **EP** — expert parallel: communicator group scoped to all ranks holding experts for one model replica. In a **MoE (Mixture-of-Experts)** layer, a learned router assigns each token to one expert — a small independent feed-forward sub-network; the full set of experts is too large to fit on one GPU, so each expert is pinned to a separate GPU. Because the destination GPU is determined at runtime by the router output (not statically at compile time), the token must be transferred to whichever GPU holds the selected expert before its forward pass can run. EP scopes that transfer to the correct replica's ranks. EP group size = DP × PCP × TP per replica; with DP=2, PCP=1, TP=16 across 32 GPUs → 2 EP groups of 16: [[0..15],[16..31]], one per DP replica.
...
Distributed inference requires different communication patterns for different parallelism axes. A **pipeline stage** is a contiguous slice of the model's layers assigned to one GPU in a pipeline-parallel setup. Within a single pipeline stage, **tensor-parallel (TP) ranks** each hold a column slice of that stage's weight matrices and must cooperate — via all-reduce — to produce the full layer output before the activation is passed to the next stage. Each GPU holds only a column slice of the weight matrix and computes a partial dot product — a **weight-parallel operation** — so those partial results must be summed before the next layer can proceed. TP ranks do this with **all-reduce**: every rank sends its partial result, the values are summed element-wise across all ranks, and each rank receives the full result.
...
**Phase 1 — control-plane (CPU, pre-GPU):** vLLM's startup routine `init_distributed_environment` issues a barrier before `torch.cuda.set_device` has been called. `set_device` creates a **CUDA context** — the GPU driver state that must exist before any GPU kernel can run; without it, GPU memory allocation fails immediately. NCCL requires a live CUDA context because its initialization allocates a per-communicator **staging buffer** — a fixed GPU memory region reserved at communicator creation to temporarily hold tensors during collective operations. Without a live CUDA context, that allocation fails. Gloo runs entirely on CPU with no CUDA dependency, so it handles this early barrier instead.

**Phase 2 — data-plane (GPU, forward pass):** Once GPUs are initialized, NCCL takes over. It issues GPU kernels directly over NVLink/PCIe, bypassing the CPU entirely. Gloo, by contrast, routes through host memory and TCP/shared-memory transports — each collective requires a CPU copy in and out of host memory, stalling on that copy. That copy overhead is acceptable for a single control-plane barrier but too slow for the 8 MiB activation tensors all-reduced every forward pass (`[batch=1, seq=512, hidden=8192]` bf16: `1×512×8192×2 = 8,388,608 bytes` across TP=32 ranks).
...
**Weight sharding** uses `rank_in_group` from the TP group because weight shards are indexed by position within the TP group — shard 0 goes to the first rank in the group, not the first rank on the node. Rank 12 belongs to TP group [12,13,14,15] and is first in that list, so `rank_in_group=0`; but it is the fifth GPU on its node, so `local_rank=4`. Using `local_rank` here loads the fifth shard instead of the first — the wrong weight slice, silently. This bug is masked on ranks 0–3: they form the first TP group and sit at the start of the first node, so `local_rank` and `rank_in_group` coincide there and the wrong attribute produces the right answer by accident.
...
PT-MoE resolves this by constructing a **cross-track group** per **slot index** — where slot index is a rank's position within its own track's rank list (0–3 for TP=4). Each track has exactly one rank at each slot index, so collecting all slot-0 ranks across 8 tracks yields exactly one rank per track: [0,4,8,12,16,20,24,28]. This cross-track group contains one representative from each track, ordered by track number. Rank 12 sits at slot 0 of track 3 (12 % 4 == 0), so it is the fourth entry in this cross-track group — `rank_in_group=3`. PT-MoE uses that value as the track index because `rank_in_group` within the cross-track group counts tracks across the full 32-GPU fleet, independent of node boundaries, while `rank` and `local_rank` do not.
