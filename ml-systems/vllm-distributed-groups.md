...
- **EP** — expert parallel: ranks holding different expert sub-networks in a **MoE** (Mixture-of-Experts) layer, where input tokens are routed to different specialized feed-forward sub-networks rather than one shared network
- **DCP** — disaggregated-context parallel: ranks partitioning the KV context across a disaggregated prefill/decode split
...
`local_rank` and `rank_in_group` diverge whenever a group doesn't start at rank 0 — the common case for any non-trivial topology. Rank 12: `local_rank=4` (node boundary at 8), `rank_in_group=0` (group [12,13,14,15] starts at 12). Using the wrong attribute produces a silent failure because each attribute answers a different question: `local_rank` answers "which GPU slot on this node?", while `rank_in_group` answers "what position am I within this communicator's rank list?" Two concrete cases show where the wrong choice corrupts behavior.
...
**PT-MoE track assignment** also uses `rank_in_group`, but from a different group: the **cross-track group**. After rebuilding into 8 tracks × TP=4, each track's forward pass is independent — ranks in different tracks process different inputs and must not share all-reduce results. A rank needs to know *which track it belongs to*, but `local_rank` is node-scoped and `rank` is job-scoped — neither encodes track membership. PT-MoE solves this by forming a **cross-track group** for each **slot index** (a rank's position within its track's rank list, 0–3 for TP=4), grouping the same slot across all 8 tracks. Slot 0 of every track forms [0,4,8,12,16,20,24,28]. Rank 12 is at slot 0 of track 3, so it is the fourth entry in this cross-track group → `rank_in_group=3`. PT-MoE uses this value as the **track index** because `rank_in_group` within the cross-track group counts track membership across the full 32-GPU fleet, independent of node boundaries.
