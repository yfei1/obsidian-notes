...
**Phase 2 — data-plane (GPU, forward pass):** Once GPUs are initialized, NCCL takes over for all data-plane collectives (all-reduce, broadcast) — it issues GPU kernels over NVLink/PCIe, bypassing the CPU entirely. Gloo routes through host memory and TCP/shared-memory transports, stalling on each CPU copy — too slow for activation all-reduces during the forward pass. Using a single communicator for both phases forces a bad choice: either block the early CPU barrier until a GPU context exists, or allocate a staging buffer for a communicator that only ever runs CPU operations. Neither is acceptable, so each logical group gets one communicator of each type.
