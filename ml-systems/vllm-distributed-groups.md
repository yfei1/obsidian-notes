...
Distributed inference requires different communication patterns for different parallelism axes. A **pipeline stage** is a contiguous slice of the model's layers assigned to one GPU in a pipeline-parallel setup. TP ranks **all-reduce** activations within a stage — all-reduce sums a tensor across all ranks and distributes the result to each. This is required after each **weight-parallel operation**: a matrix multiply whose weight matrix is split across GPUs, so each GPU computes only a partial result. In the 32-GPU example with hidden=8192, each GPU holds a `[8192, 256]` column slice (8192÷32=256 columns) and computes a partial dot product `[seq, 8192] × [8192, 256] → [seq, 256]`; those 32 partial results are all-reduced to produce the full `[seq, 8192]` output before the next layer proceeds. PP ranks send activations forward from one stage to the next; DP ranks synchronize gradients across **replicas** (identical copies of the model running on separate GPU sets, each processing different input batches).
