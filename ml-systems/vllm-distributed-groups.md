...
- **EP** — expert parallel: ranks holding different expert sub-networks in a **MoE (Mixture-of-Experts)** layer. A MoE layer replaces the standard feed-forward sub-layer with many specialist sub-networks called **experts** (each a small independent feed-forward network). A learned **router** — a small linear layer trained alongside the model — selects exactly one expert per token. Different tokens can therefore be routed to different GPUs, so EP assigns each expert to a different GPU. Routing a token to its expert may require a cross-GPU transfer before that token's forward pass proceeds.
- **DCP** — disaggregated-context parallel: ranks that move KV cache entries between GPU pools. The **KV cache** stores intermediate key/value tensors from prior tokens to avoid recomputing them (see [[ml-systems/kv-cache-internals]]). **Disaggregated serving** splits generation into two phases: **prefill** (computing all token representations from the prompt in one forward pass) and **decode** (generating each subsequent token one step at a time). Each phase runs on a separate GPU pool on separate machines. Neither pool can read the other's GPU memory directly, so the KV cache must be explicitly transferred between pools. DCP is the communicator group that performs that transfer.
...
- **`local_rank`**: physical GPU slot on this node. `torchrun` — PyTorch's distributed launcher, which starts one process per GPU and assigns each a node-local index — sets this value before the job starts. The index counts from 0 within the node, not across the full job. Rank 12 on a node covering GPUs 8–15 has `local_rank = 12 - 8 = 4`
...
PT-MoE resolves this by constructing a **cross-track group** per **slot index** — where slot index is a rank's position within its own track's rank list (0–3 for TP=4). Each track has exactly one rank at each slot index. Collecting all slot-0 ranks across 8 tracks yields [0,4,8,12,16,20,24,28] — one rank per track, covering the full fleet. Rank 12 sits at slot 0 of track 3 (12 % 4 == 0), so it is the fourth entry in this cross-track group — `rank_in_group=3`. PT-MoE uses that value as the track index because it counts track membership across the full 32-GPU fleet, independent of node boundaries.
...
**Config/actual divergence after rebuild:** `parallel_config.tensor_parallel_size` (32) and `get_tp_group().world_size` (4) diverge because the config is set once at startup and never updated. Any code that reads the config to size data structures — rather than querying the live group — computes the wrong value. KV cache head allocation is the failure case: with 32 **KV heads** (the per-head key and value projections stored in the KV cache — see [[ml-systems/kv-cache-internals]]), config TP=32 allocates 32/32=1 head/GPU. But actual TP=4 means each GPU's attention kernel writes 32/4=8 heads — 7 heads per layer land in unallocated memory, silently corrupting the cache (7×512×128×2 = 917,504 bytes ≈ 0.875 MiB overrun per layer per GPU at seq=512, head_dim=128, bf16).
