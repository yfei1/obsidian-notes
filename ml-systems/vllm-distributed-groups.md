...
- **EP** — expert parallel: ranks holding different expert sub-networks in a **MoE (Mixture-of-Experts)** layer — a layer where each input token is routed to one specialist feed-forward network instead of one shared network. EP assigns each specialist sub-network to a different GPU.
- **DCP** — disaggregated-context parallel: ranks that synchronize the KV cache across a **disaggregated prefill/decode** split. In disaggregated serving, prefill (computing the first token from a prompt) and decode (generating each subsequent token) run on separate GPU pools; their KV caches are not automatically shared, so DCP is the communicator group that transfers them across the split.
...
**Phase 1 — control-plane (CPU, pre-GPU):** `init_distributed_environment` — vLLM's startup routine that establishes process groups before any model code runs — issues a barrier before `torch.cuda.set_device` has been called. At this point no **CUDA context** (the GPU driver state required before any GPU kernel can run) exists yet. NCCL requires an active CUDA context plus a dedicated GPU memory allocation per communicator, so it cannot be used here. Gloo runs entirely on CPU with no CUDA dependency, so it handles these early barriers.
