...
- **DCP** — disaggregated-context parallel: ranks that move KV cache entries between GPU pools. In **disaggregated serving**, prefill (computing the first token from a prompt) and decode (generating each subsequent token) run on separate GPU pools. Because those pools do not share memory, the **KV cache entries** — the saved key and value tensors from prior attention steps — must be explicitly transferred. DCP is the communicator group that performs that transfer.
