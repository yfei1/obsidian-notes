...
- **EP** — expert parallel: ranks holding different expert sub-networks in a **MoE (Mixture-of-Experts)** layer. A MoE layer replaces the feed-forward sub-layer inside a transformer block with many specialist sub-networks called **experts** — a transformer block is the repeated unit of attention + feed-forward that stacks to form the full model. A learned router selects exactly one expert per token. EP assigns each expert to a different GPU. When the router sends a token to an expert on a remote GPU, that token must be transferred there before the forward pass can proceed.
- **DCP** — disaggregated-context parallel: ranks that move KV cache entries between GPU pools. The **KV cache** stores intermediate key/value tensors from prior tokens to avoid recomputing them (see [[ml-systems/kv-cache-internals]]). In **disaggregated serving**, prefill (computing the first token from a prompt) and decode (generating each subsequent token) run on separate GPU pools that do not share memory. The KV cache must therefore be explicitly transferred between pools; DCP is the communicator group that performs that transfer.
...
**Phase 1 — control-plane (CPU, pre-GPU):** `init_distributed_environment` — vLLM's startup routine that establishes process groups before any model code runs — issues a barrier before `torch.cuda.set_device` has been called. At this point no **CUDA context** exists yet; a CUDA context is the GPU driver state that must be initialized before any GPU kernel can run, and `set_device` triggers that initialization. NCCL requires an active CUDA context before it can be used — so it cannot handle this early barrier. NCCL also requires a per-communicator **staging buffer** — a fixed region of GPU memory reserved at communicator creation to temporarily hold tensors during collective operations — which cannot be allocated before the CUDA context exists. Gloo runs entirely on CPU with no CUDA dependency, so it handles these early barriers.
...
- **`local_rank`**: physical GPU slot on this node. `torchrun` — PyTorch's distributed launcher — assigns each process a node-local GPU index before the job starts. Rank 12 on a node covering GPUs 8–15 has `local_rank = 12 - 8 = 4`
