...
- **EP** — expert parallel: ranks holding different expert sub-networks in a **MoE (Mixture-of-Experts)** layer. A transformer block is the repeated unit (attention + feed-forward) that stacks to form the full model; a MoE layer replaces the feed-forward sub-layer with many specialist sub-networks called **experts** (each a small independent feed-forward network). A learned **router** — a small linear layer trained alongside the model — selects exactly one expert per token, so different tokens may need different GPUs. EP assigns each expert to a different GPU; routing a token to its expert may therefore require a cross-GPU transfer before the forward pass can proceed.
- **DCP** — disaggregated-context parallel: ranks that move KV cache entries between GPU pools. The **KV cache** stores intermediate key/value tensors from prior tokens to avoid recomputing them (see [[ml-systems/kv-cache-internals]]). **Disaggregated serving** splits generation into two phases — **prefill** (computing all token representations from the prompt in one forward pass) and **decode** (generating each subsequent token one step at a time) — and runs each phase on a separate GPU pool. Those pools are physically separate allocations on separate machines, so they cannot read each other's GPU memory directly. The KV cache must therefore be explicitly transferred between pools; DCP is the communicator group that performs that transfer.
...
Distributed inference requires different communication patterns for different parallelism axes. A **pipeline stage** is a contiguous slice of the model's layers assigned to one GPU in a pipeline-parallel setup. TP ranks **all-reduce** activations within a stage — all-reduce sums a tensor across all ranks and distributes the result to each. This is required after each **weight-parallel operation** — a matrix multiply whose weight matrix is split across GPUs. In the 32-GPU example with hidden=8192, each GPU holds a `[8192, 256]` column slice (8192÷32=256 columns) and computes a partial dot product `[seq, 8192] × [8192, 256] → [seq, 256]`; those 32 partial results are all-reduced to produce the full `[seq, 8192]` output before the next layer proceeds. PP ranks send activations forward from one stage to the next; DP ranks synchronize gradients across **replicas** (identical copies of the model running on separate GPU sets, each processing different input batches).
...
**Phase 1 — control-plane (CPU, pre-GPU):** `init_distributed_environment` — vLLM's startup routine that establishes process groups before any model code runs — issues a barrier before `torch.cuda.set_device` has been called. `set_device` creates a **CUDA context** — the GPU driver state required before any GPU kernel can run. That context does not exist yet at this point, so NCCL cannot initialize: NCCL requires an active CUDA context plus a per-communicator **staging buffer** (a fixed GPU memory region reserved at communicator creation to temporarily hold tensors during collective operations), and neither can exist before `set_device`. Gloo runs entirely on CPU with no CUDA dependency, so it handles these early barriers.
