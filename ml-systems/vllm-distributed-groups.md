...
- **EP** — expert parallel: ranks holding different **expert sub-networks** — the distinct feed-forward specialists in a MoE (Mixture-of-Experts) layer, where input tokens are routed to one specialist rather than passing through one shared network
- **DCP** — disaggregated-context parallel: ranks partitioning the KV cache in a **disaggregated prefill/decode** architecture — a serving topology where prefill (computing the first token from a prompt) and decode (generating each subsequent token) run on separate GPU pools, so their KV caches must be explicitly synchronized across the split
