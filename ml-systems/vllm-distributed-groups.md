...
- **EP** — expert parallel: ranks holding different expert sub-networks in a **MoE (Mixture-of-Experts)** layer. A MoE layer is a drop-in replacement for the feed-forward sub-layer inside a transformer block — a transformer block is the repeated unit of attention + feed-forward that stacks to form the full model — where instead of one feed-forward network there are many specialist sub-networks called **experts**. A learned router selects exactly one expert per token. EP assigns each expert to a different GPU, so when the router sends a token to an expert on a remote GPU, that token must be transferred there before the forward pass can proceed.
- **DCP** — disaggregated-context parallel: ranks that move KV cache entries between GPU pools. In **disaggregated serving**, prefill (computing the first token from a prompt) and decode (generating each subsequent token) run on separate GPU pools. Because those pools do not share memory, the **KV cache** — the saved intermediate tensors from prior tokens that avoid recomputation (see [[ml-systems/kv-cache-internals]]) — must be explicitly transferred between pools. DCP is the communicator group that performs that transfer.
...
Distributed inference requires different communication patterns for different parallelism axes. A **pipeline stage** is a contiguous slice of the model's layers assigned to one GPU in a pipeline-parallel setup. TP ranks **all-reduce** (sum a tensor across all ranks and distribute the result) activations after each **weight-parallel operation** — a matrix multiply whose weight matrix is split across GPUs, requiring the partial results to be summed before the next layer — within a stage; PP ranks send activations forward from one stage to the next; DP ranks synchronize gradients across **replicas** (identical copies of the model running on separate GPU sets, each processing different input batches).
...
**Phase 1 — control-plane (CPU, pre-GPU):** `init_distributed_environment` — vLLM's startup routine that establishes process groups before any model code runs — issues a barrier before `torch.cuda.set_device` has been called. At this point no **CUDA context** exists yet; a CUDA context is the GPU driver state that must be initialized before any GPU kernel can run, and `set_device` triggers that initialization. NCCL requires an active CUDA context plus a dedicated per-communicator **staging buffer** — a fixed region of GPU memory reserved at communicator creation to temporarily hold tensors during collective operations — so it cannot be used before `set_device`. Gloo runs entirely on CPU with no CUDA dependency, so it handles these early barriers.

**Phase 2 — data-plane (GPU, forward pass):** Once GPUs are initialized, NCCL takes over for all data-plane collectives — it issues GPU kernels over NVLink/PCIe, bypassing the CPU entirely. A concrete case: all-reducing a `[1, 512, 8192]` bf16 activation tensor (`1×512×8192×2 = 8,388,608 bytes = 8 MiB`) across TP=32 ranks every forward pass. Gloo routes through host memory and TCP/shared-memory transports, stalling on each CPU copy — too slow for 8 MiB activation all-reduces at inference latency. Using a single communicator for both phases forces a bad choice: either block the early CPU barrier until a GPU context exists, or allocate a staging buffer for a communicator that only ever runs CPU operations. Neither is acceptable, so each logical group gets one communicator of each type.
...
`torch.distributed.new_group` is a **collective barrier** — every rank in the entire job must call it before any rank proceeds, regardless of whether that rank belongs to the group being created. This is because NCCL's communicator setup requires a globally synchronized handshake: the library must exchange device topology information and the GPU buffer addresses (the staging buffers allocated at communicator creation) across all participants before the communicator is usable. That handshake waits for every rank to check in, so skipping the call on even one rank stalls all others indefinitely. The constructor (parallel_state.py:316-395) therefore iterates ALL group rank lists, calling `new_group` twice per group (once for NCCL, once for Gloo), and stores only the group the current rank belongs to. For TP=32 that is 1 group × 2 backends = 2 calls per rank; after the PT-MoE rebuild into 8 tracks × TP=4 it is 8 groups × 2 backends = 16 calls per rank.
...
- **`local_rank`**: physical GPU slot on this node, set by `torchrun` (PyTorch's distributed launcher, which assigns each process a node-local GPU index before the job starts) — rank 12 on a node covering GPUs 8–15 has `local_rank = 12 - 8 = 4`
...
The three attributes are scoped differently — job, node, group — and both weight sharding and track assignment require `rank_in_group`, but from *different* groups.
...
**PT-MoE track assignment** also needs `rank_in_group`, but from the **cross-track group** (defined below) rather than the TP group. The rebuilt TP group encodes which 4 GPUs share a forward pass, but not which track number (0–7) a rank belongs to across the full fleet. After rebuilding into 8 tracks × TP=4, each track runs an independent forward pass on different inputs. `rank` is job-scoped and `local_rank` is node-scoped — neither directly encodes which of the 8 tracks a rank belongs to.
...
**Config/actual divergence after rebuild:** `parallel_config.tensor_parallel_size` (32) and `get_tp_group().world_size` (4) diverge because the config is set once at startup and never updated. Any code that reads the config to size data structures — rather than querying the live group — computes the wrong value. KV cache head allocation is the critical case: with 32 **KV heads** (the per-head key and value projections stored in the KV cache — see [[ml-systems/kv-cache-internals]]), config TP=32 allocates 32/32=1 head/GPU. But actual TP=4 means each GPU's attention kernel writes 32/4=8 heads — 7 heads per layer land in unallocated memory, silently corrupting the cache.
