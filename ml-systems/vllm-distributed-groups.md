...
- **DCP** — disaggregated-context parallel: ranks that move KV cache entries between GPU pools. The **KV cache** stores intermediate key/value tensors from prior tokens to avoid recomputing them (see [[ml-systems/kv-cache-internals]]). In **disaggregated serving**, prefill (computing the first token from a prompt) and decode (generating each subsequent token) run on separate GPU pools that do not share memory — the pools cannot read each other's GPU memory directly. The KV cache must therefore be explicitly transferred between pools; DCP is the communicator group that performs that transfer.
...
PT-MoE resolves this by constructing a **cross-track group** per **slot index** — a rank's position within its own track's rank list (0–3 for TP=4). Each slot value appears exactly once per track, so collecting slot-0 ranks across all tracks yields [0,4,8,12,16,20,24,28]. Rank 12 sits at slot 0 of track 3, so it is the fourth entry in this cross-track group — `rank_in_group=3`. PT-MoE uses that value as the track index because it counts track membership across the full 32-GPU fleet, independent of node boundaries.
