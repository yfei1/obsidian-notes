...
- **EP** — expert parallel: ranks holding different expert sub-networks in a **MoE (Mixture-of-Experts)** layer. A MoE layer replaces one shared feed-forward network with many specialist sub-networks — each input token is routed to exactly one specialist. EP assigns each specialist to a different GPU. When a token is routed to a specialist on a remote GPU, it must be sent there before the forward pass can proceed.
- **DCP** — disaggregated-context parallel: ranks that move KV cache entries between GPU pools. In **disaggregated serving**, prefill (computing the first token from a prompt) and decode (generating each subsequent token) run on separate GPU pools. Because those pools do not share memory, the **KV cache entries** — the saved key and value tensors that attention computed for each prior token (see [[ml-systems/kv-cache-internals]]) — must be explicitly transferred. DCP is the communicator group that performs that transfer.
...
Distributed inference requires different communication patterns for different parallelism axes. A **pipeline stage** is a contiguous slice of the model's layers assigned to one GPU in a pipeline-parallel setup. TP ranks **all-reduce** (sum a tensor across all ranks and distribute the result) activations after each weight-parallel operation within a stage; PP ranks send activations forward from one stage to the next; DP ranks synchronize gradients across **replicas** (identical copies of the model running on separate GPU sets, each processing different input batches).
