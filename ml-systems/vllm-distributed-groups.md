...
All three attributes are scoped differently — job, node, group — so using the wrong one produces a silent failure: the right type of index, but counting from the wrong origin.
