...
- **EP** — expert parallel: communicator group scoped to all ranks holding experts for one model replica. In a **MoE (Mixture-of-Experts)** layer, a learned router assigns each token to one expert (a small independent feed-forward sub-network); because experts are pinned to separate GPUs so the full expert set fits in aggregate memory, routing requires a cross-GPU transfer before each token's forward pass. EP scopes that transfer to the correct replica's ranks.
