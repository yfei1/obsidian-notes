...
**Phase 1 — control-plane (CPU, pre-GPU):** `init_distributed_environment` — vLLM's startup routine that establishes process groups before any model code runs — issues a barrier before `torch.cuda.set_device` has been called. `set_device` is the call that creates a **CUDA context** — the GPU driver state required before any GPU kernel can run. At this point that context does not exist yet. NCCL requires an active CUDA context before it can initialize — so it cannot handle this early barrier. NCCL also requires a per-communicator **staging buffer** — a fixed GPU memory region reserved at communicator creation to temporarily hold tensors during collective operations — which cannot be allocated without a CUDA context. Gloo runs entirely on CPU with no CUDA dependency, so it handles these early barriers.
