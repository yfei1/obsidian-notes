...
- **EP** — expert parallel: ranks holding different expert sub-networks in a **MoE (Mixture-of-Experts)** layer. A MoE layer replaces one shared feed-forward network with many specialist sub-networks; each input token is routed to one specialist instead of passing through a single shared network. EP assigns each specialist sub-network to a different GPU — so when a token is routed to a specialist that lives on a different GPU, it must be sent there before the forward pass can proceed.
- **DCP** — disaggregated-context parallel: ranks that transfer the KV cache across a **disaggregated prefill/decode** split. In disaggregated serving, prefill (computing the first token from a prompt) and decode (generating each subsequent token) run on separate GPU pools. Because the two pools do not share memory, DCP is the communicator group that moves **KV cache entries** — the saved key and value tensors from prior attention steps — from the prefill pool to the decode pool.
...
Distributed inference requires different communication patterns for different parallelism axes. A **pipeline stage** is a contiguous slice of the model's layers assigned to one GPU in a pipeline-parallel setup. TP ranks **all-reduce** (sum a tensor across all ranks and distribute the result) activations within a stage; PP ranks send activations forward from one stage to the next; DP ranks synchronize gradients across **replicas** (identical copies of the model running on separate GPU sets, each processing different input batches).

Each pattern needs its own **NCCL communicator** — a named handle to a fixed set of GPUs that can issue collectives among themselves — scoped to exactly the right ranks. A communicator scoped to the wrong rank set either silently excludes GPUs or deadlocks: NCCL waits for every rank in the communicator to call the collective, so a rank that was never meant to participate never calls in, and the operation hangs. **vLLM solves this with one 5D rank tensor (`ExternalDP × DP × PP × PCP × TP`) from which all six groups are derived by transpose + reshape** — each view of the same data produces the correct rank lists for one parallelism axis without redundant bookkeeping.
...
**PT-MoE track assignment** also needs `rank_in_group`, but from the **cross-track group** (defined below) rather than the TP group, because the rebuilt TP group does not encode which track a rank belongs to. After rebuilding into 8 tracks × TP=4, each track runs an independent forward pass on different inputs. An all-reduce must stay within one track's 4 GPUs — `rank` is job-scoped and `local_rank` is node-scoped, so neither directly encodes track membership.
...
**Config/actual divergence after rebuild:** `parallel_config.tensor_parallel_size` (32) and `get_tp_group().world_size` (4) diverge because the config is set once at startup and never updated. Any code that reads the config to size data structures — rather than querying the live group — computes the wrong value. KV cache head allocation is the critical case: with 32 **KV heads** (the key and value projection heads in multi-head attention — one per attention head in the standard case), config TP=32 allocates 32/32=1 head/GPU. But actual TP=4 means each GPU's attention kernel writes 32/4=8 heads — 7 heads per layer land in unallocated memory, silently corrupting the cache.
