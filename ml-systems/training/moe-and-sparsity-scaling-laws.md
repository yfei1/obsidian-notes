...
3. **How does Tencent Hunyuan-Large determine the optimal activated parameter count for MoE?**
   Hunyuan-Large applies Chinchilla IsoFLOP quadratic fitting specifically to **activated parameters** on small compute budgets ($5.0 \times 10^{18}\text{--}9.5 \times 10^{19}$ FLOPs). Plotting the resulting minima on log-log axes produces a linear power-law regression that extrapolates across ~4.5 orders of magnitude to predict an optimal activated parameter size of 58.1B at $C_{\min} \approx 3.11 \times 10^{24}$ FLOPs (with Hunyuan selecting 52B in practice due to quadratic minimum flatness).
