...
3. **On-Device Hardware Envelope (arXiv:2404.06395 Appendix E)**: On-device SLMs target mobile edge deployment; after INT4 quantization (Appendix D), MiniCPM-2.4B's memory footprint compresses to 2GB for on-device inference (e.g. iPhone 15 Pro at 18 tokens/s, Table 10). Under strict smartphone memory limits, the Chinchilla assumption that $N$ and $D$ scale freely together is physically invalid.
...
4. **Why do edge models like MiniCPM train to $450N$ tokens instead of stopping at Chinchilla's $20N$?**
   Chinchilla's $20N$ rule minimizes one-off training FLOPs under the assumption that parameter count $N$ can scale freely alongside data $D$. Edge devices have strict memory limits; MiniCPM targets mobile deployment where INT4 quantization compresses the 2.4B model to 2GB (Appendix E Table 10). With $N$ fixed by the hardware memory envelope, loss decreases monotonically with additional tokens, making overtraining past 20N (into 450N) the only way to maximize capability per byte of RAM.
