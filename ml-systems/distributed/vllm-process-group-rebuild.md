...
Prerequisite: [[ml-systems/distributed/vllm-distributed-groups]] — group layout, `GroupCoordinator` internals, rank attributes.
...
Fix: store truthful per-track values in the config and patch the methods that divide by global TP to use `within_track_tp = tp // num_tracks` instead. Computed attributes that depend on mutable state cannot survive Pydantic nesting. PT-MoE's plugin applies this fix — see [[ml-systems/vllm/pt-moe-vllm-implementation]].
...
- [[ml-systems/distributed/vllm-distributed-groups]] — group layout tensor, `GroupCoordinator` internals, rank attributes; this note extends that with runtime mutation semantics
- [[ml-systems/vllm/pt-moe-vllm-implementation]] — PT-MoE plugin that applies the rebuild pattern and the config fix described here
- [[ml-systems/vllm/vllm-weight-loading]] — weight loading uses `rank_in_group` for track slicing, affected by post-rebuild group state
- [[ml-systems/inference/kv-cache-internals]] — KV cache buffer allocation that silently corrupts when fed stale TP size
