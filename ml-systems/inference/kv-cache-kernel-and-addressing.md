...
**Prerequisites**: [[ml-systems/inference/kv-cache-internals]] (6-D tensor layout, block table, slot addressing), [[ml-systems/gpu/gpu-memory-hierarchy]] (why CPU pre-computation avoids GPU division cost), [[ml-systems/foundations/attention-mechanics]] (prefill vs decode attention paths), [[ml-systems/gpu/gpu-kernel-stack]] (Triton DSL, thread block model, tl.program_id semantics).
...
See [[ml-systems/inference/kv-cache-internals]] for the 6-D tensor layout, cache sizing, and per-layer slice assignment that this kernel operates on.
...
def store_kvcache_kernel(key_ptr, key_stride, value_ptr, value_stride,
    ...
    if slot == -1: return                         # prefix-cached token (K/V already in cache) → skip; see [[ml-systems/inference/prefix-caching]]
...
o = flash_attn_varlen_func(q, k, v, ...)               # FlashAttention (fused attention kernel, see [[ml-systems/foundations/attention-mechanics]]) — local k, v, NOT the cache
...
- [[ml-systems/inference/kv-cache-internals]] — 6-D tensor layout, cache sizing via warmup, per-layer slice assignment (prerequisites for this note)
- [[ml-systems/foundations/attention-mechanics]] — flash_attn API details, prefill vs decode kernel signatures
- [[ml-systems/inference/flashinfer-vllm-integration]] — production FlashInfer kernels that replace the nano-vLLM `flash_attn_with_kvcache` calls shown here
- [[ml-systems/gpu/gpu-memory-hierarchy]] — why slot_mapping is pre-computed on CPU (memory wall, division cost)
- [[ml-systems/inference/prefix-caching]] — when `slot == -1` (skip) and when `block_tables` is set during prefill
- [[ml-systems/inference/prefix-caching-hash-table-leak]] — implementation bug where the hash→block_id map in BlockManager grows unbounded because stale entries are never purged on block recycling
- [[ml-systems/gpu/gpu-kernel-stack]] — Triton sits within the broader GPU kernel stack; `store_kvcache_kernel` is a concrete example of a custom Triton kernel replacing a CUDA primitive
- [[ml-systems/vllm/vllm-model-integration]] — vLLM's model runner (the production counterpart to nano-vLLM's `model_runner.py`) is where slot_mapping is assembled before kernel launch
