...
**Scope**: nano-vLLM's KV cache implementation — tensor layout, VRAM sizing, per-layer view assignment, and the Triton write kernel. For the attention math that motivates the cache, see [[ml-systems/foundations/attention-mechanics]]; for the BlockManager and PagedAttention abstraction, see [[ml-systems/inference/llm-inference-engines]].

**Prerequisites**: [[ml-systems/foundations/attention-mechanics]] (Q/K/V projections, prefill vs. decode), [[ml-systems/gpu/gpu-memory-hierarchy]] (VRAM hierarchy, warp coalescing), [[ml-systems/foundations/transformer-model-internals]] (decoder layer structure).
...
Full kernel code, `slot_mapping` computation, memory contiguity proof, and prefill/decode call-site details: [[ml-systems/inference/kv-cache-kernel-and-addressing]].
...
# See [[ml-systems/foundations/parallel-track-architecture]] for within_track_tp details
```

Concrete values for Qwen3-0.6B on a single GPU (TP=1):
- `config.num_key_value_heads = 8` <!-- source: Qwen3-0.6B config.json -->
- `within_track_tp = 1` (`within_track_tp`: the TP degree for this layer's track — see [[ml-systems/foundations/parallel-track-architecture]]; single GPU here, so no head splitting)
...
Because the cache reads from layers (Phase 6 in [[ml-systems/distributed/vllm-distributed-groups]]), not from config (Phase 2), models that modify their effective TP inside `__init__()` automatically get correctly-sized KV cache — no manual edits to the checkpoint config are needed to fix the head count for cache sizing.
...
- [[ml-systems/gpu/gpu-memory-hierarchy]] — memory wall, why slot_mapping is pre-computed on CPU
- [[ml-systems/inference/prefix-caching]] — hash-based block reuse, when block_tables is set during prefill
- [[ml-systems/foundations/transformer-model-internals]] — where KV cache fits in the decoder layer data flow
- [[ml-systems/distributed/vllm-distributed-groups]] — full startup lifecycle showing when cache is allocated relative to group rebuild
- [[ml-systems/inference/flashinfer-vllm-integration]] — alternative attention backend that also consumes slot_mapping and block_tables
- [[ml-systems/inference/prefix-caching-hash-table-leak]] — block reuse bug rooted in BlockManager hash-table lifecycle
- [[ml-systems/inference/cuda-graph-inference-optimization]] — how static slot_mapping tensors interact with CUDA graph capture
- [[ml-systems/gpu/gpu-kernel-stack]] — PTX/Triton compilation pipeline that produces the store_kvcache kernel
- [[ml-systems/inference/kv-cache-kernel-and-addressing]] — Triton kernel that writes K/V vectors using flat 1D slot addressing pre-computed by the CPU scheduler
