...
**Scope**: How vLLM serves multiple LoRA adapters on one base model — the `SupportsLoRA` contract, layer wrapping, adapter loading, and per-request routing via Punica kernels. Does not cover LoRA theory or training — see [[ml-systems/foundations/lora-mechanics]] for the concept.

**Prerequisites**: [[ml-systems/foundations/lora-mechanics]] (low-rank factorization, adapter format), [[ml-systems/vllm/vllm-model-integration]] (model registration, weight loading), [[ml-systems/foundations/transformer-model-internals]] (linear layers).
...
Because they're registered as `torch.ops.vllm.*` custom ops with in-place mutation, they appear as **opaque nodes** in torch.compile's FX graph (see [[ml-systems/gpu/gpu-kernel-stack]] for how custom ops interact with Inductor). The base matmul `W @ x` and the LoRA correction `B @ A @ x` cannot be fused into one kernel — each forward pass through a LoRA-wrapped layer is 3 separate kernel launches.

---

## LoRA + Tensor Parallelism

LoRA weights are sliced at load time to match the base model's TP configuration — the same column/row slicing pattern used for all vLLM weights (see [[ml-systems/distributed/parallelism-strategies]]). No custom handling needed:
...
**Shape mismatch**: `set_lora()` copies adapter weights into pre-allocated stacked tensors sized at model startup. If the adapter was trained on a different hidden size or number of heads, the copy fails because the pre-allocated slot shape doesn't match — there is no dynamic resizing at load time.

**Adapter slot exhaustion**: `max_loras` (default 1) caps simultaneous resident adapters because each slot occupies GPU memory pre-allocated at startup. When all slots are full and a new request needs an unloaded adapter, `LRUCacheWorkerLoRAManager` evicts the least-recently-used adapter — freeing its slot by overwriting it with the new adapter's weights.

**Naming mismatch**: Adapter tensor names must match the model's `named_modules()` paths exactly because `set_lora()` looks up each layer by name to find its pre-allocated slot. Adapters trained with PEFT use the convention `base_model.model.model.layers.0.self_attn.qkv_proj.lora_A.weight` — models with non-standard layer naming need a `WeightsMapper` to translate before lookup.
...
- [[ml-systems/foundations/lora-mechanics]] — LoRA concept: low-rank factorization, alpha scaling, adapter format
- [[ml-systems/vllm/vllm-model-integration]] — model registration contract, weight loading
- [[ml-systems/vllm/vllm-torch-compile-decorator]] — `@support_torch_compile` and opaque custom ops
- [[ml-systems/gpu/gpu-kernel-stack]] — Triton, custom ops, CUDA graphs
- [[ml-systems/foundations/mixture-of-experts]] — FusedMoE architecture, expert parallelism
- [[ml-systems/distributed/parallelism-strategies]] — TP column/row slicing patterns
## Connections

- [[ml-systems/gpu/pytorch-module-hooks]] — LoRA adapter swapping relies on `nn.Module` hook and `__call__` dispatch to redirect weight reads without modifying `forward()` directly
