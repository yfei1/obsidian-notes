...
The Python engine separates these phases because the underlying CUDA kernels configure GPU SRAM differently for each — tiling for compute-bound prefill vs split-K for memory-bound decode. See [[ml-systems/gpu/gpu-memory-hierarchy]] for the tiling vs split-K deep dive.
...
See [[ml-systems/inference/cuda-graph-inference-optimization]] for the full treatment: graph capture phases, why graphs are decode-only (prefill padding wastes ~99% of tensor cores), pinned memory vs UVA transfer patterns, and garbage collection pitfalls.
