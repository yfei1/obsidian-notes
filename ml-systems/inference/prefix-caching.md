...
**Scope**: How nano-vLLM implements content-addressable KV cache blocks — hash chaining, allocation, prefill skip, deallocation, and the unbounded `hash_to_block_id` growth bug.

**Prerequisites**: [[ml-systems/inference/kv-cache-internals]] (block structure, ref counting, free-list mechanics), [[ml-systems/foundations/attention-mechanics]] (prefill vs. decode, cu_seqlens), [[ml-systems/gpu/gpu-memory-hierarchy]] (block allocation model), [[ml-systems/vllm/vllm-executor-architecture]] (BlockManager lifecycle and scheduler integration)

## Core Intuition

**Every request that shares a system prompt recomputes the same KV cache from scratch — wasting the GPU work already done for a prior request.** Prefill attention cost is O(S²) because each query token attends to all prior tokens; a 512-token shared prefix means 512 × 512 attention operations on *every* new request. Prefix caching eliminates this by making KV blocks *content-addressable*: each block is keyed by a hash of its token IDs, so the `BlockManager` detects a shared prefix at allocation time and hands the new sequence pointers to the existing blocks instead of allocating fresh ones. On a hit, the attention kernel reads cached K/V from those shared blocks and computes new Q/K/V only for the uncached suffix — turning a 512-token prefill into a 16-token one, a **32× reduction** in tokens sent to the GPU (31× in QK^T FLOPs, because the cached K still spans the full context length).

---

## TL;DR

Prefix caching lets the engine skip recomputing KV cache for token blocks it has already seen. A KV cache block is a fixed-size chunk of pre-allocated GPU memory that stores Key and Value vectors for a group of tokens (see [[ml-systems/gpu/gpu-memory-hierarchy]]). nano-vLLM uses **content-based hash chaining** — each block of tokens gets an xxhash (a fast, non-cryptographic hash function) of its token IDs, and that hash is mixed with the previous block's hash to form a chain. Chaining means block *i*'s hash encodes the entire prefix through block *i*, not just the tokens in block *i* alone — so two sequences only share a cache hit if their prefixes are identical up to that block. On allocation, the `BlockManager` looks up the hash in `hash_to_block_id` (a dict mapping hash → physical block ID); on a hit it reuses the existing KV cache block, skipping GPU work for those tokens. `hash_to_block_id` never shrinks — the "Why Does `hash_to_block_id` Grow Without Bound?" section below explains how stale entries accumulate and why they are functionally harmless.
...
Block 2's hash therefore encodes blocks 0+1+2's entire content. Two sequences only share a cache hit if their **entire prefix** is identical up to that block. Partial (last) blocks — blocks with fewer than `block_size` tokens, i.e., the final block of a sequence that doesn't fill completely — always get `h = -1` and are never cached.
...
The attention kernel tracks two lengths: `cu_seqlens_q` (how many tokens are being *computed* this step) and `cu_seqlens_k` (how many tokens the keys must span, including any cached prefix). For a cold prefill, both equal 512 — every token is computed fresh, so Q and K cover the same range.

When `cu_seqlens_q == cu_seqlens_k`, the kernel takes the **in-place path**: Q, K, and V are produced and consumed within a single contiguous allocation. No `block_table` (a per-sequence array mapping logical block indices to physical GPU block IDs) is needed because all K/V is written fresh in this pass — there are no prior blocks to look up. The kernel computes 512 × 512 attention and writes 32 × 2 MB = 64 MB of KV data into the 32 allocated blocks.
...
Seq 2 shares the same 512-token system prompt (32 blocks). For each shared block, `compute_hash` produces the same chain hash — because the token IDs are identical — so `hash_to_block_id` returns seq 1's block ID. The token ID validation (`self.blocks[block_id].token_ids != token_ids`) passes, confirming the hit:
...
Because `seq.num_cached_tokens = 512`, `prepare_prefill` sends only the 16-token suffix to the GPU:
...
This creates a `cu_seqlens` mismatch: `cu_seqlens_q` = 16 (tokens being *computed*) but `cu_seqlens_k` = 528 (full context the keys must span, including the cached prefix). The divergence matters because the in-place kernel path — used during cold prefill — assumes Q and K cover the same tokens, so it never consults a `block_table`. With `cu_seqlens_k > cu_seqlens_q`, the kernel must read cached K/V for tokens 0–511 from the 32 shared blocks, which it can only reach via `block_tables`. The engine detects the mismatch and passes them:
...
With `block_tables` in hand, the kernel reads cached K/V from the 32 shared blocks while computing new Q/K/V only for the 16 suffix tokens. GPU work drops from 512 tokens to 16 — a **32× reduction** in prefill compute.

The reduction is concrete in FLOPs. Prefill attention cost per layer is dominated by the QK^T matmul: `2 × S_q × S_k × (H × d)` multiply-adds, where S_q is query length, S_k is key length, H = 8 KV heads, d = 128 head-dim. Cold vs. cached:
...
- **Ratio**: 536,870,912 / 17,301,504 ≈ **31×** fewer QK^T multiply-adds (close to 32×; S_k = 528 not 512 for the cached case accounts for the difference)

| | Prompt 1 (cold) | Prompt 2 (32 hits) |
|---|---|---|
| Tokens sent to GPU | 512 | 16 |
| KV blocks allocated | 32 fresh | 1 fresh + 32 shared |
| Prefill compute | 512-token QK^T attention | 16-token QK^T attention (31× fewer ops) |
...
The engine must answer two questions at deallocation time: (1) is this block still in use by another sequence, and (2) if not, should its KV data be discarded?

**ref_count > 1**: the block is shared — seq 2 still holds a reference to the same physical block as seq 1. Freeing it would corrupt seq 2's cached KV data, so the block stays allocated.

**ref_count == 0**: the block is unowned, but the engine does *not* discard its KV data. It moves the block to the **free list** (pool of unowned blocks available for reuse) while keeping its hash, token_ids, and KV cache data intact — because a future request with the same prefix can reclaim the block without a prefill pass, saving ~8 ms per 512-token system prompt.
...
When a block hits ref_count 0, it moves to `free_block_ids` but **keeps its hash, token_ids, and KV cache data** — the next request hitting the same prefix reclaims all 32 blocks via `_allocate_block(block_id)` in the cache-hit path, bypassing prefill entirely. Shared blocks (ref_count > 1) survive because seq 2 still holds a reference and the condition `block.ref_count == 0` is never reached.
...
The problem is purely memory: the dict grows proportionally to the number of distinct prefixes seen, with no eviction. See [[ml-systems/inference/prefix-caching-hash-table-leak]] for the full trace, stale-entry analysis, and correct fix patterns.
...
4. **"Any bugs in this design?"** — `hash_to_block_id` grows without bound. See [[ml-systems/inference/prefix-caching-hash-table-leak]] for the full trace and fix analysis.

---

## See Also

- [[ml-systems/inference/kv-cache-internals]] — block structure, ref counting, and free-list mechanics this note builds on
- [[ml-systems/foundations/attention-mechanics]] — how `cu_seqlens_q` vs `cu_seqlens_k` diverge during cached prefill
- [[ml-systems/gpu/gpu-memory-hierarchy]] — physical block layout and memory budget calculations
- [[ml-systems/inference/llm-inference-engines]] — where prefix caching fits in the broader serving pipeline
- [[ml-systems/vllm/vllm-model-integration]] — how `block_tables` are passed to the attention kernel
- [[ml-systems/foundations/transformer-model-internals]] — KV head dimensions and layer counts used in the running example
- [[ml-systems/inference/kv-cache-kernel-and-addressing]] — physical slot mapping, how block_table indices translate to GPU memory addresses, and kernel-level handling where `slot == -1` skips the write and `block_tables` redirects prefill attention reads to cached K/V
- [[ml-systems/inference/flashinfer-vllm-integration]] — the variable-length attention kernel that reads cached KV via block_tables during prefill
- [[ml-systems/inference/prefix-caching-hash-table-leak]] — deep dive into the unbounded-growth bug and correct fix patterns
- [[ml-systems/vllm/vllm-executor-architecture]] — the executor drives the scheduler and BlockManager that prefix caching hooks into
- [[ml-systems/inference/cuda-graph-inference-optimization]] — CUDA graph capture must account for the variable-length prefill path that prefix cache hits produce
