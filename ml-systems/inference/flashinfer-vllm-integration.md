...
**FlashInfer vs FlashAttention-2**: FlashAttention-2 (implemented in Triton — a Python DSL that compiles to GPU PTX (NVIDIA's intermediate assembly language, one level above machine code) assembly) is vLLM's default backend because it has wider GPU support and Python-readable source. FlashInfer is not the default because its paged-layout benefit only matters when gather cost is non-trivial: for prefill-only or contiguous-KV workloads, there are no scattered blocks to gather, so the gather cost is zero and FlashAttention-2's simplicity wins. FlashInfer takes over when decode with long context makes gather-before-attend dominate — on Llama-3-8B with 512 FP16 blocks, that is 32 MB copied per forward pass before a single multiply-accumulate runs — or when FP8/FP4 quantized KV is needed, a capability FlashAttention-2 does not provide.
