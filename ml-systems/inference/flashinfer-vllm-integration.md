...
FlashInfer is a library of hand-written C++/CUDA kernels for attention, sampling, MoE, and collective communication. vLLM uses it at five integration points: paged attention (prefill + decode), FP4 MoE, top-k/top-p sampling, all-reduce, and MLA decode. FlashInfer kernels are registered as **opaque custom ops** via `torch.library` (PyTorch's API for registering kernels that the compiler treats as black boxes) — meaning dynamo (torch.compile's tracing frontend, which walks Python bytecode to build a computation graph) records each kernel as a single graph node without inspecting its internals. CUDA graph capture requires every operation in the recorded sequence to be a fixed, static kernel launch with no Python-level branching — opaque nodes satisfy this by construction, since dynamo never tries to trace inside them. This makes them compatible with **CUDA graph capture** — pre-recording a fixed sequence of GPU commands once, then replaying the recording on subsequent steps without CPU overhead. The key advantage over FlashAttention-2: native paged KV cache support and FP8/FP4 quantization built into the kernel.

---

## What This Component Does

FlashInfer solves a performance problem: standard attention implementations do not understand paged memory layouts (where KV cache blocks are scattered across GPU memory, like virtual memory pages — see [[ml-systems/inference/kv-cache-internals]]). A naive kernel would first gather (copy) those scattered blocks into one contiguous buffer, wasting memory bandwidth before a single multiply-accumulate runs. FlashInfer fuses the page-table lookup into the attention kernel itself, reading directly from scattered blocks.
...
See [[ml-systems/foundations/mixture-of-experts]] for MoE architecture details.
...
MLA (Multi-head Latent Attention, used in DeepSeek-V2/V3) compresses KV into a lower-rank latent space — see [[ml-systems/foundations/attention-mechanics]] for the full mechanism. The FlashInfer MLA kernel handles the latent-space decode natively. It is restricted to SM 10.x (SM = streaming multiprocessor compute capability; 10.x = Hopper/Blackwell) because it requires `qk_nope_head_dim == 128` (line 85).
...
See [[ml-systems/foundations/attention-mechanics]] for attention variants.
...
**FlashInfer vs FlashAttention-2**: vLLM defaults to FlashAttention-2 for two reasons: it supports a wider range of GPUs, and its Triton source (a Python DSL that compiles to PTX — NVIDIA's intermediate assembly language) is debuggable without profiling tools. FlashInfer's paged-layout advantage — reading directly from scattered KV blocks rather than gathering them first — only pays off when those gather costs are non-trivial. For prefill-only or contiguous-KV workloads, there are no scattered blocks, so gather cost is zero and FlashAttention-2's simplicity wins. FlashInfer becomes the better choice in two cases: (1) long-context decode where gather-before-attend dominates — on Llama-3-8B with 512 FP16 blocks, that is 32 MB copied per forward pass before a single multiply-accumulate runs; (2) FP8/FP4 quantized KV, which FlashAttention-2 does not support.

**CUDA graph interaction**: CUDA graph capture records a fixed sequence of GPU commands, so every tensor shape and kernel argument must be static at capture time. FlashInfer's `plan()` call embeds batch-specific metadata — sequence lengths, block table pointers — directly into the recorded command stream, which means a wrapper captured for batch=4 cannot replay correctly for batch=8: the block table pointer and length arrays baked in at capture time are wrong for a different batch. vLLM solves this by pre-creating one wrapper per captured batch size (line 532-534), because each size needs its own static recording. The memory cost is non-trivial: the default capture set {1, 2, 4, 8, 16, 32} produces 6 wrapper instances, each holding a 128 MB workspace buffer <!-- source: FlashInfer BatchDecodeWithPagedKVCacheWrapper default workspace_size_in_bytes --> — 768 MB total on a 40 GB A100 <!-- source: NVIDIA A100 datasheet --> where KV cache competes for the same pool.
...
**Sampling sync cost**: FlashInfer's rejection sampling must check a stopping condition on the CPU — accept or redraw — which forces a CPU-GPU sync not present in vLLM's default `random_sample` path. That sync stalls the decode loop because the CPU cannot issue the next kernel until the GPU confirms acceptance. vLLM therefore only activates FlashInfer sampling when top-k or top-p filtering is explicitly requested (gated behind `VLLM_USE_FLASHINFER_SAMPLER=1`, line 39); for greedy or unfiltered random sampling, the default path is faster because it never syncs.

**Relationship to Triton and torch.compile**: FlashInfer and hand-written Triton kernels occupy the same position in the stack (see [[ml-systems/gpu/gpu-kernel-stack]]): both register as `torch.library` custom ops, appear as single opaque nodes in the FX graph, and are compatible with CUDA graph capture. The difference is implementation language — FlashInfer kernels are C++/CUDA; Triton kernels are Python compiled to PTX. This matters for debugging because the opaque boundary hides internals from the compiler: a perf regression inside a FlashInfer kernel is invisible to Inductor's fusion log and requires nsight or nvprof to diagnose, whereas a Triton kernel's source is Python-readable. Inductor (torch.compile's code-generation backend, which fuses and lowers the FX graph to GPU kernels) fuses element-wise ops *around* both, never *into* them — the opaque boundary is the same in both cases.

---

## Interview Talking Points

1. **"What is FlashInfer and why does vLLM use it?"** -- The cost of NOT using it: on Llama-3-8B with 512 FP16 blocks, a gather-then-attend kernel copies 32 MB per forward pass before a single multiply-accumulate runs. FP8 KV halves that to 16 MB — but only if the kernel understands the paged layout natively.

2. **"How does FlashInfer relate to torch.compile?"** -- The boundary matters for debugging: Inductor fuses ops *around* the FlashInfer node but never *into* it. A perf regression inside the kernel won't show up in the Inductor fusion log — you need nsight or nvprof to see it. Same applies to Triton custom ops.
...
4. **"What are the five FlashInfer integration points in vLLM?"** -- (1) Paged attention for prefill + decode via `BatchPrefillWithPagedKVCacheWrapper` and `BatchDecodeWithPagedKVCacheWrapper`, (2) FP4 MoE via `trtllm_fp4_block_scale_routed_moe`, (3) top-k/top-p sampling via rejection sampling, (4) fused all-reduce via `flashinfer.comm`, (5) MLA decode via `trtllm_batch_decode_with_kv_cache_mla`. Hidden cost: CUDA graph capture requires one wrapper per batch size — default 6 sizes × 128 MB workspace = 768 MB on a 40 GB A100.

---

## See Also

- [[ml-systems/gpu/gpu-kernel-stack]] -- how Triton, torch.compile, and CUDA graphs compose; FlashInfer sits at the same custom-op level as Triton
- [[ml-systems/foundations/mixture-of-experts]] -- MoE architecture and FusedMoE kernel details
- [[ml-systems/foundations/attention-mechanics]] -- attention variants including MHA, GQA, MQA, MLA
- [[ml-systems/inference/kv-cache-internals]] -- paged KV cache layout that FlashInfer reads natively
- [[ml-systems/inference/kv-cache-kernel-and-addressing]]
- [[ml-systems/inference/prefix-caching]]
- [[ml-systems/inference/cuda-graph-inference-optimization]] — FlashInfer attention kernels are captured inside CUDA graphs during decode; the static tensor shapes required for graph capture constrain how FlashInfer receives sequence metadata
- [[ml-systems/inference/kv-cache-kernel-and-addressing]] — nano-vLLM baseline kernel (`flash_attn_with_kvcache`) that FlashInfer replaces in production
- [[ml-systems/foundations/gqa-mqa-attention-variants]] — conceptual foundations of MQA, GQA, and DeepSeek Multi-Head Latent Attention (MLA)
