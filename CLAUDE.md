...
- **Topic directories** group related notes: `ml-systems/`, `data-processing/`, etc.
...
- **Never buffer output**: Always use `tee` instead of `tail`/`head` when piping loop output (e.g. `| tee /tmp/run.log`). `tail -N` buffers the entire stream until EOF. `PYTHONUNBUFFERED=1` only unbuffers Python — downstream pipe commands can still buffer.
